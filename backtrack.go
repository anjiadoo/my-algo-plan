...
// 🌟技巧5：可复选vs不可复选 - 可复选问题（如combinationSum）递归时传递当前索引i而不是i+1，不可复选问题传递i+1，这一个参数的差异决定了元素能否重复使用
// 🌟技巧6：同层去重剪枝（i>start） - 含重复元素的组合/子集问题（subsetsWithDup、combinationSum2），先排序再用i>start && nums[i]==nums[i-1]跳过同层重复，保证只选第一个重复元素
...
// ⚠️易错点3：去重排序前提 - 含重复元素的排列/组合问题（permuteUnique、subsetsWithDup、combinationSum2），去重的前提是先sort.Ints(nums)排序，不排序直接去重会遗漏或出错
