...
| **<font style="color:#AD1A2B;">WAL 本地事务日志</font>** | <font style="color:#AD1A2B;">LPOP 成功后立即写本地磁盘日志，再发 MQ</font> | 进程 crash 时 MQ 未发送，重启后从 WAL 重放；<font style="color:#AD1A2B;">不用DB替代是因为 crash 时 DB 连接也可能断</font> |
| **DB 唯一索引作最终兜底** | grab_record 表 uk_packet_uid (red_packet_id, uid) | Redis 主从切换数据丢失时，即使同一金额被再次弹出，DB 层 duplicate key 阻止重复入账 |
| **发包List完整性校验** | 写完后立即 LLEN 校验，不等于 total_count 则清空重试 | OOM/网络中断导致 List 只写一半，校验+重试保证"要么全写成功，要么全不写" |
...
| **MQ 削峰控制 DB 写入速率** | 抢包峰值 20万 TPS经 MQ 削峰，消费侧受控在2万TPS | DB直连抢包链路（20万TPS）需67个主库；MQ削峰后只需16个主库，节约4倍成本 |
...
| **DB 写入峰值** | **20万 TPS（Redis侧瞬时）**<br/>**2万 TPS（DB消费侧受控）** | Redis LPOP 成功瞬时产生 20万抢中事件/s；经 MQ 削峰后，消费侧以 2万 TPS 持续写 DB |


### 容量计算
**带宽：**

+ 入口计算：1000万 QPS × 1KB/请求 × 8bit ÷ 1024³ ≈ **80 Gbps**
+ 入口规划：80 Gbps × 2（冗余系数）= **160 Gbps**  
依据：2倍冗余应对 TCP 重传、协议头开销、网络抖动及突发流量，为公网入口通用标准
+ 出口计算：1000万 QPS × 512B × 8bit ÷ 1024³ ≈ 40 Gbps，规划 **80 Gbps**（同样2倍冗余）
...
| **数据** | 计算过程 | 估算结果 | 说明 |
| :---: | --- | --- | --- |
| **红包主表** | 20万包/s × 86400s × 200B/条 ÷ 1024⁴ | **≈ 3.4 TB/天** | 单条红包记录约 200B（ID/类型/金额/状态/时间字段），春节全天峰值估算 |
| **抢包记录** | 200万次/s × 86400s × 100B/条 ÷ 1024⁴ | **≈ 17 TB/天** | 单条抢包记录约 100B，以峰值 QPS 全天估算（最大值，实际偏低） |
| **Redis 热数据** | 见下方拆解 | **≈ 50 GB** | 含主从复制缓冲区、内存碎片、扩容余量 |
| **MQ 消息** | 2万/s × 86400s × 1KB/条 × 3天÷1024³ | **≈ 5 TB** | 取全天均值 2万/s（MQ 消费侧控速，非 Redis 峰值速率，见下方说明） |
...
+ **总热数据**：500万 × 6KB ÷ 1024² ≈ 29GB（1024² = 1GB，将 KB 转换为 GB）
...
| 服务 | 单机QPS | 依据 | 有效 QPS | 节点数计算 | 节点数 |
| --- | --- | --- | --- | --- | --- |
| 抢包服务 | 1500 | 含本地缓存判断 + Redis Lua + WAL写入 + MQ发送，链路中等 | 200万 | 200万 ÷ (1500 × 0.7) ≈ 1905 | **取2000台** |
...
> 冗余系数取 **0.7**：服务节点负载不超过70%，预留 GC 停顿、流量毛刺、节点故障摘流、弹性扩容窗口余量。
>
...
+ 入账 Topic 同步刷盘（<font style="color:#AD1A2B;">金融强保障</font>），其余 Topic 异步刷盘（性能优先）
...
| :---: | --- | --- | --- | --- |
| **RedPacket** 红包（聚合根） | 红包全生命周期：创建→抢中→抢完/过期→退款完成；**内部包含"预拆槽位"子项** | 红包ID、发包人ID、总金额（分）、总个数、状态、过期时间；**子项 Slots[]**：槽位序号、该份金额（分）、抢包人ID | 创建红包（二倍均值法预拆）、Lua 原子抢包（消费槽位）、标记抢完/过期 | Redis List + Hash 为权威源（发包时写入，抢包时 LPOP）+ MySQL 持久化兜底（防Redis丢失重建） |
...
| :---: | --- | --- | --- | --- |
| **PacketSent** <br/>发包事件 | 红包创建成功后触发 | 红包ID、发包人ID、总金额（分）、幂等请求ID | SendTransaction 提交 + 资金扣款成功 | ① 账户流水写入（扣款流水）② 会话消息推送（群红包气泡） |
| **PacketGrabbed**<br/>抢包事件 | 用户抢中红包 | 红包ID、抢包人ID、抢到金额（分）、槽位序号、抢包时间戳 | Redis Lua 抢包成功 + GrabRecord 写入 | ① **入账流水**（记账到用户余额） ② 欧皇/手气王推送 |
| **PacketExpired**<br/>过期事件 | 红包到期未抢完 | 红包ID、发包人ID、剩余金额、剩余个数 | 过期扫描任务发现 + 状态机切换 | ① 生成退款任务 <br/>② 通知发包人 |
| **RefundCompleted**<br/>退款完成事件 | 退款资金回到发包人账户 | 红包ID、发包人ID、退款金额（分） | 退款执行成功（含重试最终成功） | ① 退款流水写入 ② 通知发包人 ③ 对账视图更新 |
...
| :---: | --- | --- | --- | --- |
| **AccountFlow**<br/>账户流水<br/>（审计视图） | 完整资金流水账本：扣款/入账/退款 | 流水ID、用户ID、流水类型（扣款/入账/退款）、金额、业务ID(红包ID)、处理状态、创建时间 | 消费 `PacketSent/PacketGrabbed/RefundCompleted` 事件，每个事件至少一条流水 | **强一致最终账本**，对账必须 100% 吻合 |
| **PacketDetailView** 红包详情视图 | 展示红包状态、手气王、抢包列表 | 红包ID、总金额、已抢金额、已抢个数、手气王用户ID、抢包列表 | Redis Hash缓存红包摘要 + ZSet存抢包顺序，事件驱动更新 | 最终一致（允许秒级延迟） |
...
| 模型 | 职责 | 归属 |
| :---: | --- | --- |
| **SendTransaction**<br/>发包事务表 | 发包幂等（uk_request_id）+ MQ发送补偿 | 基础设施：本地事务表，解决"红包扣款 + MQ 发送"的跨资源一致性 |
| **RefundTask**<br/>退款任务表 | 退款进度追踪、失败重试、对账兜底 | 基础设施：消费 `PacketExpired` 事件落地的任务队列，独立于业务聚合 |


> 这两张表在各大高并发系统中都以不同名字出现（秒杀的 `seckill_transaction`、feed 的 `FanoutTask`），它们是**基础设施层的事务/任务补偿机制**，不是业务领域模型。把它们列出来是为了说清楚 "发包扣款如何保证落 DB + MQ 不丢" 以及"过期后退款如何可靠执行"。
>
...
+ **写路径极简**：发包只有"扣款 + 预拆 + 事件"，抢包只有"Lua原子消费 + GrabRecord记录 + 事件"。
+ **事件必然对应流水**：金融合规要求每次资金变动必有 AccountFlow，通过事务消息保证不丢。
+ **Redis权威 + MySQL兜底**：抢包热点链路靠 Redis，MySQL 只做持久化兜底（重建 Redis / 对账）
...
+ LVS 负责全局流量分发与链路转发；
+ 云原生网关承担 SSL 卸载、IP黑名单拦截、设备指纹校验、全局限流（1200万 QPS 上限）、请求路由分发；
...
+ **发包服务**：处理发包幂等校验、账户预扣款、二倍均值法拆包、DB事务写入、List预热、MQ通知推送；
+ **抢包服务**：本地缓存前置拦截、Redis Lua原子抢包、WAL事务日志写入、MQ事务消息发送；
+ **查询与入账服务**：MQ消费落库、账户入账、红包详情查询、过期退款触发；
...
+ **Redis 集群**：<font style="color:#AD1A2B;">普通红包集群</font>（16分片，每分片1主2从，AOF+RDB 持久化）+ <font style="color:#AD1A2B;">热点红包专用集群</font>（独立1主2从，物理隔离），承载库存 List、已抢用户 Set、红包元信息 Hash；
+ **RocketMQ 集群**：8主8从 + 4个NameServer，<font style="color:#AD1A2B;">入账Topic同步刷盘</font>，其余异步刷盘，承载抢包落库、入账触发、过期退款、发包通知等全部异步消息；
...
**预拆金额 + Redis LPOP**：发包时预计算所有份额写入 List，抢包时 LPOP 原子弹出，天然防超抢无需分布式锁
...
③ 发包服务
   ...
   │    容灾：Redis失败不影响发包成功 ⚠️
   │
   ├─ 6. 发送 MQ topic_send_notify 群通知
   │
   └─ 7. 响应前端，返回 red_packet_id
...
③ 异步写DB + 入账（脱离抢包关键路径，不影响响应速度）
        ...
        ├─ 消费者消费消息，执行以下操作：
        │   1. 幂等写入 grab_record 表（uk_packet_uid 唯一索引兜底防重）
        │   2. 写入 account_flow 表（状态=处理中）
        │   3. 调用账户服务，执行用户入账操作
        │   4. 更新 account_flow 表状态=成功
        └─ 异步执行，不阻塞下一步
        │
        ▼
...
| Key | 说明 | 类型 | 大小估算 | TTL |
| :---: | :---: | :---: | :---: | :---: |
| `rp:slots:{id}` | 红包槽位，拆包后的各个红包份额 | List | 200人×4B = 800B | 25h |
| `rp:meta:{id}` | 红包元信息，如红包金额、个数等 | Hash | ~200B | 25h |
| `rp:grab:{id}` | 已抢用户集合 | Set | 200人×8B = 1.6KB | 25h |
| `rp:detail:{id}` | 抢包明细(uid->amount)，供详情页 | Hash | 200人×16B = 3.2KB | 25h |
| **单红包总计** | **** | — | **~6KB** | — |
| **500万活跃红包** | **** | — | **~30GB** | — |
...
| Topic | 消费者 | 用途 | 峰值消息速率 | 分区数计算 | 刷盘策略 |
| --- | --- | --- | --- | --- | --- |
| `topic_grab_record` | 入账服务 | 抢包记录落库+入账触发 | 20万条/s（每次成功抢包发一条） | 20万 ÷ (5000 × 0.7) ≈ 57，取2的幂次 | 同步刷盘 |
| `topic_send_notify` | 通知服务 | 发包成功通知群成员 | 20万条/s（每个红包发包后通知群成员，与发包速率一致） | 20万 ÷ (8000 × 0.7) ≈ 36，取2的幂次 | 异步刷盘 |
| `topic_expire_refund` | 退款服务 | 过期退款：金融操作，不允许丢 | 约 1000条/s（过期红包占比低，远小于发包速率） | 1000 ÷ (3000 × 0.7) ≈ 0.5，最低保障高可用取 | 同步刷盘 |
| `topic_grab_stat` | 统计服务 | 抢包统计、排行 | 20万条/s（与抢包速率一致，允许延迟消费） | 20万 ÷ (8000 × 0.7) ≈ 36，取2的幂次且允许堆积降配 | 异步刷盘 |
| `topic_dead_letter` | 告警服务+人工 | 死信兜底 | 极低（仅重试耗尽的异常消息） | 无需高吞吐，保障多节点消费即可 | 同步刷盘 |
...
**分片数设计**：热点红包流量 / Redis 单分片安全 QPS = N；例：1000万 QPS/10万 = 100个分片，独立集群承载
...
**问题分析：**  
Redis 默认异步复制，<font style="color:#DF2A3F;">主库 LPOP 成功但未同步到从库期间主库宕机，从库切主后 List 恢复到未被弹出的状态，导致"已弹出给用户的金额"又被新主库再次弹出</font>——即**同一份金额被两个用户抢到**，属于超抢。
...
### Q2：抢包成功后先返回给用户，DB 异步落库——如果此时 DB 连接池耗尽或 MQ 堆积，导致 grab_record 长时间未写入，用户重复刷"未到账"，怎么保证最终一致且不重复入账？
**参考答案：**
...
### Q3：春节 0 点发红包，同一秒有 20 万个红包同时发出，大量用户同一时间收到"有新红包"通知，如何设计通知推送不造成"通知风暴"压垮系统？
**参考答案：**
...
### Q4：预拆金额方案的弱点——发包后 24 小时才过期，但红包被抢完后 Redis List 已空，这期间 LLEN=0，如何设计"实时判断红包是否抢完"比全量查询 DB 更高效？
**参考答案：**
...
![](https://cdn.nlark.com/yuque/0/2026/png/25540478/1777366874954-47598fe4-164d-4d5b-99ed-3590227d4e32.png)
...
+ 每次 LPOP 成功后，Lua 内执行 `HINCRBY rp:meta:{id} grabbed_count 1`（更新已抢计数）



---

### Q5：分库分表后抢包记录按 red_packet_id 分片，但用户想查"我发出的所有红包"和"我收到的所有红包"，跨库查询如何设计？
**参考答案：**

**核心：****<font style="color:#AD1A2B;">CQRS（命令查询分离）+ 异步冗余写</font>****，查询走独立的读视图，不跨分片查询。**
...
### Q6：如何设计一个"手气最佳"的实时展示功能——哪个人抢到了最大金额？要求无论多少人同时看，P99 < 20ms？
**参考答案：**
...
![](https://cdn.nlark.com/yuque/0/2026/png/25540478/1777367211483-c75126ee-e0af-4601-9b7c-db9823d5b65f.png)
...
### Q7：发包预扣款和抢包入账是两个独立服务操作两套账户，如何保证"发包总金额 = 入账总金额 + 退款总金额"这个金融等式在任何时刻都成立？
**参考答案：**
...
金融等式：
    ...
    - 发包池入金(type=2) = SUM(抢包出金(type=1)) + 退款出金(type=3) + 在途金额 "任意时刻"在途金额 ≠ 0，因此等式不要求实时，要求结束后等于0

② **每日全量对账（核心）**：

![](https://cdn.nlark.com/yuque/0/2026/png/25540478/1777367429878-3d2600ff-1d8a-4269-84cb-bf70efae32ea.png)
...
### Q8：Redis LPOP 弹出金额后，服务进程 crash（未发 MQ、未写 DB），该金额永久丢失——用户没抢到钱，但 Redis 库存已减 1，怎么处理这种<font style="color:#AD1A2B;">"空弹"</font>问题？
**参考答案：**

**核心：LPOP 成功后****<font style="color:#AD1A2B;">立即写本地事务日志（WAL）</font>****，crash recovery 时重放未完成事务。**
...
### Q9：如果要支持"企业红包"——公司给 10 万名员工发红包，发包时 Redis 一次性写入 10 万个 slot，这个写操作本身会造成 Redis 阻塞，怎么解决？
**参考答案：**
...
**Q10：企业级超大红包（10w 级名额）拆包需分批批量写入 Redis，若服务机器内存不足、OOM 或网络中断，分批写入中途终止，出现 DB 已扣款、红包槽位数据完整落地，但 Redis List 仅写入一半数据如何处理？**

**参考答案：**

**核心**：超大红包分批写 Redis 无法单命令原子执行，依靠「DB全量落地 + 状态锁 + 长度校验 + 定时重建」解决缓存残缺不一致问题。

**问题分析**：大额企业红包单命令写入载荷超限，必须分批循环 RPUSH；发包链路 DB 事务已完成扣款、红包主表、Slot 槽位表持久化，若分批写入中途服务 OOM 崩溃，会出现 DB 数据完整、Redis List 数据半截的异常状态。

**解决方案：**

**① 写入状态管控：**
- DB 事务提交后，先标记红包缓存状态为writing，未完全写完不切为可抢状态；领取接口优先校验状态，拦截未就绪红包，防止脏数据被消费。

**② 分批写完强制校验：**
- 全批次写入完成后，执行 LLEN 校验列表长度；长度与总名额不符时，通过 Lua 原子清空残缺 List，基于 DB 槽位数据本地重试重写。

**③ 定时兜底补偿：**
- 定时任务周期巡检，筛选DB 已完成、缓存异常 / 状态未就绪的红包；以 red_packet_slot 表为基准，批量重刷重建 Redis List，修复写入中断问题。

**④ 幂并发防护：**
- 发包事务唯一索引保证不重复扣款；重建缓存增加分布式锁，避免多节点并发重复推送，保障补写安全。
