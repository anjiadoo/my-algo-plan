...
| 章节 | 核心内容 |
|------|---------|
| **需求与边界** | 三类推送场景（业务单推/群推/运营全量广播）+ 双通道（长连在线推 + 三方离线推 APNs/FCM/厂商通道）、明确禁行（禁止全量广播回源 DB、禁止在线通道瞬时写扩散、禁止同步等待三方厂商 ACK、禁止无限重试） |
| **容量评估** | DAU 6亿闭环推导、单推峰值 80万 QPS、运营全量广播 6亿条 5分钟内推完（扇出 200万条/秒）、带宽/存储/Redis/MQ/长连接网关完整闭环 |
| **库表设计** | 按"实体+事件+读模型+基础设施"四类建模（实体 PushTask 含 Shard 子项/PushMessage/DeviceRoute、事件 PushTriggered/MessageDispatched/Acked/Failed/TaskProgressed、读模型 UserInbox/PushStatsView/DeviceRouteCache、基础设施 PushIdem/VendorToken）、7张核心表（推送任务表/消息表按 msg_id 分片/用户收件箱/设备路由表/厂商凭证表/幂等记录表/回执表），SQL 带分片规则 + 索引 + 冷热分离策略 |
| **整体架构图** | Mermaid flowchart + 五层描述（接入/网关 WebSocket/推送编排/通道适配/存储），在线推+离线推双通道核心设计原则 |
| **核心流程** | 单推写扩散（路由→通道选择→在线走长连/离线走厂商）、群推读扩散（任务分桶→并行 Worker）、运营 6亿全量广播（任务切片→Bitmap 过滤→通道池化推送）、ACK 回执闭环 |
| **缓存架构** | 四级缓存（端本地去重 → 设备路由缓存 Hash → 推送任务状态 Redis → 厂商 Token Kv），设备路由 TTL 策略、ACK Bitmap 压缩存储 |
| **MQ 设计** | 7个 Topic 分区数推导（在线推/离线推/回执/重试/死信/任务编排/用户事件），延迟消息 15min 实现"离线转在线"、消费者幂等 + 顺序性消费（保证同一 device_id 消息有序） |
| **核心关注点** | 大V群推扇出(写扩散 vs 读扩散)、全量广播限速与削峰(令牌桶+厂商通道配额)、防刷(用户维度/App维度)、幂等方案(msg_id+device_id)、消息顺序性(device 维度 Kafka Key)、去重(客户端 Bloom) |
| **容错性设计** | 分层限流(接入/厂商 QPS/用户维度)、熔断(APNs 失败率>20% 熔断)、三级降级(关闭富推送→关离线补推→关运营推送)、ETCD 动态开关、五类故障兜底矩阵 |
| **可扩展性** | Gateway 无状态+一致性哈希、Redis 按 uid 在线扩容、MQ 分区弹性扩容、任务切片并行 Worker 自动扩缩容、多地域推送节点就近接入 |
| **高可用运维** | 5大类监控(到达率/到达延迟P99/厂商通道错误率/长连在线率/回执延迟)、分级告警、大型运营推送 checklist、春节红包/618/双11 护航规范 |
| **面试高频10题** | Q1~Q10 均为推送场景定制(6亿全量广播5分钟推完/长连不在线转厂商/APNs VoIP与普通通道/同一设备消息有序/大V 5000万粉丝扇出/厂商Token失效/三方通道限速配额/推送回执链路/多设备同一账号路由/运营推送灰度) |
