...
| 章节 | 核心内容 |
|------|---------|
| **一、需求与边界** | 三类推送场景（业务单推/群推/运营全量广播）+ 双通道（长连在线推 + 三方离线推 APNs/FCM/厂商通道）、明确禁行（禁止全量广播回源 DB、禁止在线通道瞬时写扩散、禁止同步等待三方厂商 ACK、禁止无限重试） |
| **二、容量评估** | DAU 6亿闭环推导、单推峰值 80万 QPS、运营全量广播 6亿条 5分钟内推完（扇出 200万条/秒）、带宽/存储/Redis/MQ/长连接网关完整闭环 |
| **三、库表设计** | 按"实体+事件+读模型+基础设施"四类建模（实体 PushTask 含 Shard 子项/PushMessage/DeviceRoute、事件 PushTriggered/MessageDispatched/Acked/Failed/TaskProgressed、读模型 UserInbox/PushStatsView/DeviceRouteCache、基础设施 PushIdem/VendorToken）、7张核心表（推送任务表/消息表按 msg_id 分片/用户收件箱/设备路由表/厂商凭证表/幂等记录表/回执表），SQL 带分片规则 + 索引 + 冷热分离策略 |
| **四、整体架构图** | Mermaid flowchart + 五层描述（接入/网关 WebSocket/推送编排/通道适配/存储），在线推+离线推双通道核心设计原则 |
| **五、核心流程** | 单推写扩散（路由→通道选择→在线走长连/离线走厂商）、群推读扩散（任务分桶→并行 Worker）、运营 6亿全量广播（任务切片→Bitmap 过滤→通道池化推送）、ACK 回执闭环 |
| **六、缓存架构** | 四级缓存（端本地去重 → 设备路由缓存 Hash → 推送任务状态 Redis → 厂商 Token Kv），设备路由 TTL 策略、ACK Bitmap 压缩存储 |
| **七、MQ 设计** | 7个 Topic 分区数推导（在线推/离线推/回执/重试/死信/任务编排/用户事件），延迟消息 15min 实现"离线转在线"、消费者幂等 + 顺序性消费（保证同一 device_id 消息有序） |
| **八、核心关注点** | 大V群推扇出(写扩散 vs 读扩散)、全量广播限速与削峰(令牌桶+厂商通道配额)、防刷(用户维度/App维度)、幂等方案(msg_id+device_id)、消息顺序性(device 维度 Kafka Key)、去重(客户端 Bloom) |
| **九、容错性设计** | 分层限流(接入/厂商 QPS/用户维度)、熔断(APNs 失败率>20% 熔断)、三级降级(关闭富推送→关离线补推→关运营推送)、ETCD 动态开关、五类故障兜底矩阵 |
| **十、可扩展性** | Gateway 无状态+一致性哈希、Redis 按 uid 在线扩容、MQ 分区弹性扩容、任务切片并行 Worker 自动扩缩容、多地域推送节点就近接入 |
| **十一、高可用运维** | 5大类监控(到达率/到达延迟P99/厂商通道错误率/长连在线率/回执延迟)、分级告警、大型运营推送 checklist、春节红包/618/双11 护航规范 |
| **十二、面试高频10题** | Q1~Q10 均为推送场景定制(6亿全量广播5分钟推完/长连不在线转厂商/APNs VoIP与普通通道/同一设备消息有序/大V 5000万粉丝扇出/厂商Token失效/三方通道限速配额/推送回执链路/多设备同一账号路由/运营推送灰度) |
...
### 问题1：你说运营全量广播 6亿用户要 5分钟内推完，扇出 200万 QPS。但厂商通道（APNs/FCM等）有严格的 QPS 配额，怎么保证真的能在 5 分钟内推完？假如 APNs 拒绝服务怎么办？
...
### 问题2：长连 Gateway 单机 10 万连接，2.4亿在线需要 2400 台。用户发消息时，怎么找到目标 device 所在的 Gateway 节点？跨节点转发怎么保证延迟？
...
### 问题3：推送系统有"消息必达"需求（比如提现到账通知），但 Kafka 是 at-least-once，APNs 也可能丢，端侧可能没展示。你怎么设计"金融级必达"的推送？
...
### 问题4：你说同一 device 的消息要保序（Kafka Key=device_id），但业务方一次可能发 100 条消息给同一设备，如果 Gateway 消费并发度低会堆积，怎么办？
...
### 问题5：你设计了读扩散模型应对大V 5000万粉丝，但这意味着粉丝必须"主动拉"，而推送本质是"推"。假如粉丝 3 天不打开 App，5000万粉丝的消息怎么办？
...
### 问题6：厂商通道（APNs/FCM）的 Push Token 会失效（用户卸载、换设备、系统升级），不清理会导致推送任务中大量失败消耗资源。你如何设计 Token 治理？
...
### 问题7：推送系统要做灰度（比如先推 10% 用户观察到达率），大型运营推送如何平滑控制推送节奏？怎么防止"一键推送全量"导致的雪崩？
...
### 问题8：iOS APNs 有"静默推送"（content-available=1）和"通知推送"两种，Android 厂商也有类似的区分。你怎么利用这些特性优化用户体验？
...
### 问题9：业务方接入推送系统后，经常投诉"推送延迟"、"推送丢失"、"推送重复"。作为平台方，你怎么快速定位问题？监控体系怎么建？
...
### 问题10：推送系统是典型的"削峰填谷"场景，业务相对均衡但运营广播集中在固定时段（比如早 8 点 / 晚 8 点推早晚报）。你如何做成本优化？K8s 弹性扩缩容怎么落地？
