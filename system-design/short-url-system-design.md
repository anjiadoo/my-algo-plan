...
| 章节 | 核心内容 |
|------|---------|
| **需求与边界** | 功能边界（生成/跳转/统计/管理）、明确禁行（禁止 DB 直连跳转、禁止同步写统计） |
| **容量评估** | 100万 QPS × 86400s 闭环验证、带宽/存储逐步推导、Redis 热数据 450GB 详细拆解、DB 分库数推导、RocketMQ 16主16从推导 |
| **库表设计** | 6张核心表（短链主表/幂等表/访问日志/统计表/封禁表/序列表），含分库分表策略 |
| **整体架构图** | Mermaid flowchart + 分层文字描述（4层：接入/应用/中间件/存储） |
| **核心流程** | 号段模式短码生成（ETCD CAS + Base62 + Feistel 置换防枚举）、三级缓存跳转（含完整 Go 代码）、点击统计异步流程 |
| **缓存架构** | 布隆过滤器双重用途、singleflight 防击穿、TTL 抖动防雪崩、主动续期防毛刺 |
| **MQ 设计** | 4个 Topic 分区数推导（含为何点击统计可用异步刷盘的工程取舍说明） |
| **核心关注点** | 唯一性三重保障、热点分级（普通/热点/超级热点）、Feistel 防枚举、安全扫描 |
| **容错性设计** | 分层限流/熔断/三级降级/ETCD 开关/兜底矩阵 |
| **可扩展性** | 布隆过滤器重建方案（区别于普通 Redis 扩容）、ClickHouse 冷热分层 |
| **高可用运维** | 监控指标（含 `su_short_code_collision_total=0` 这类安全级指标）、大流量预案 |
| **面试高频10题** | Q1~Q10 均为短链场景定制，无通用模板（号段浪费/缓存毛刺/布隆误判/301vs302/访问限制/来源分析/UV去重/冗余存储/自定义域名/跨分片查询） |
