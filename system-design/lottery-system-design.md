...
| **MySQL 对账 + ClickHouse 概率监控** | 定时 Redis vs MySQL 库存对账 + ClickHouse 实时产出率监控告警 | Redis 主从切换可能丢数据；概率偏离可能因 Bug/作弊导致——双重兜底保证数据正确、概率公平。ClickHouse 列式存储支撑 5亿条/天明细数据的秒级聚合分析 |
...
| 统计明细 (ClickHouse) | 5亿条/天 × 300B/条 | **150 GB/天（压缩后约30 GB/天）** | 列式压缩比5:1~10:1，保留1年明细 |
...
**Kafka 集群（统计管道）：**
- 单 Broker 安全吞吐：50万条/s（异步刷盘，日志流场景）
- 所需 Broker：50万 ÷ 50万 = 1，取 **3 Broker**（高可用副本因子=3）
- Topic: LOTTERY_STAT，64 分区，retention=72h
- Consumer: Flink 集群实时消费写入 ClickHouse
...
| 事件 | 触发时机 | 下游消费 |
|------|---------|---------|
| **DrawCompleted** 抽奖完成 | 本地事务提交成功 | ① 异步发货(RocketMQ) ② 写抽奖流水(RocketMQ→MySQL) ③ 统计上报(Kafka→ClickHouse) |
...
│                        中间件层                                       │
│  ┌──────────────┐ ┌──────────────┐ ┌──────────┐ ┌──────────────┐  │
│  │ Redis Cluster│ │  RocketMQ    │ │  Kafka   │ │    ETCD      │  │
│  │ 32分片1主2从  │ │ 16主16从      │ │ 统计管道  │ │  配置中心     │  │
│  └──────────────┘ └──────────────┘ └──────────┘ └──────────────┘  │
└─────────────────────────────┬───────────────────────────────────────┘
                              │
┌─────────────────────────────▼───────────────────────────────────────┐
│                        存储层                                         │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ │
│  │MySQL 16库 │ │ClickHouse│ │ ES 日志   │ │Prometheus│ │  HDFS    │ │
│  │按月分表   │ │统计分析    │ │链路追踪   │ │+ Grafana │ │ 冷数据   │ │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────┘ │
└─────────────────────────────────────────────────────────────────────┘
```
...
| Topic | MQ | 峰值消息速率 | 分区数计算 | 分区数 | 刷盘策略 | 用途 | 消费者 |
|-------|-----|------------|-----------|--------|---------|------|--------|
| `LOTTERY_DELIVERY` | RocketMQ | 50万条/s | 50万÷(5000×0.7)≈143, 取2的幂次 | **64** | 同步刷盘 | 抽奖发货（道具不能丢，需事务消息） | 发货服务 → MySQL |
| `LOTTERY_RECORD` | RocketMQ | 50万条/s | 50万÷(8000×0.7)≈89, 取2的幂次 | **32** | 异步刷盘 | 抽奖流水异步落库 | 记录服务 → MySQL |
| `LOTTERY_BROADCAST` | RocketMQ | 1万条/s | 远低于单分区吞吐, 保障高可用取 | **8** | 异步刷盘 | 大奖全服广播（需延迟消息支持定时广播） | 通知服务 |
| `LOTTERY_STAT` | Kafka | 50万条/s | 日志流管道，高吞吐优先 | **64** | 异步刷盘 | 概率统计/产出监控/用户行为分析 | Flink → ClickHouse |
| `LOTTERY_DLQ` | RocketMQ | 极低 | 保障多节点消费 | **4** | 同步刷盘 | 死信兜底 | 告警+人工 |

> **为何 LOTTERY_DELIVERY 用同步刷盘？** 道具=用户资产，消息丢失=用户花了钱没拿到东西（P0事故），牺牲20%写性能换资产安全。
> **为何 LOTTERY_DELIVERY 和 LOTTERY_RECORD 用 RocketMQ？** 发货依赖事务消息（半消息+回查）保证"扣费↔发货"原子性，流水落库配合本地事务保证不丢。
> **为何 LOTTERY_STAT 用 Kafka？** 统计数据是典型大数据管道场景（高吞吐日志流 → OLAP），不需要事务消息/延迟消息，Kafka 单 Broker 吞吐远高于 RocketMQ（百万级 vs 十万级），天然适配 Flink/Spark 消费写入 ClickHouse。
...
L4 ClickHouse（OLAP 统计分析）:
   └── 抽奖流水明细、概率统计、产出监控、用户行为分析 —— Kafka实时写入，支撑实时大屏和概率审计
```
...
温数据（7~90天） : MySQL 归档库（读写分离）+ ClickHouse（统计分析/概率监控/实时大屏）
冷数据（>90天）  : 导入 HDFS / TiDB（海量历史离线分析）
迁移策略：定时任务每日凌晨将超过7天的 draw_record 搬运到归档库
统计数据全量实时写入 ClickHouse（Kafka → Flink → ClickHouse，保留1年明细供概率审计）
```
...
抽奖请求 → draw_id 生成
    │
    ├─ draw_record 表：draw_id → 完整抽奖快照（MySQL）
    ├─ draw_transaction 表：draw_id → 事务状态（MySQL）
    ├─ RocketMQ 消息 Keys：draw_id → 消息轨迹
    ├─ delivery_record 表：draw_id → 发货状态（MySQL）
    ├─ ES 日志：draw_id → 全链路日志（请求/响应/中间状态，适合字段级检索）
    └─ ClickHouse：draw_id → 统计分析（概率偏离/产出率/用户行为聚合）

排查工具：
  输入 draw_id → ES 一键查询全链路状态（抽奖结果/事务状态/MQ投递/发货结果）
  输入 user_id + time_range → ClickHouse 查询该用户所有抽奖统计（产出分布/保底触发/消费分析）
```
...
场景3：概率实时监控（按小时桶，Redis做秒级粗略统计）
  Key: stat:{pool_id}:{hour}
  HINCRBY total_draws 1
  HINCRBY ssr_hits 1
  实际SSR率 = ssr_hits / total_draws（秒级告警判断）
  注：精确统计分析（分布/百分位/TOP-N/趋势）走 ClickHouse
```
