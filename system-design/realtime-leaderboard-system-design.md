...
| 章节 | 核心内容 |
|------|---------|
| **需求与边界** | 功能边界（得分上报/排名查询/多维榜单/历史快照/反作弊）、明确禁行（禁止 ORDER BY 实时排序、禁止好友榜上主链路、禁止历史快照阻塞写入） |
| **容量评估** | 100万 QPS 上报 × 3倍写入放大闭环验证、带宽/存储逐步推导、Redis 热数据 200GB 详细拆解（全局榜/地区榜/好友榜缓存分别计算）、DB 32库推导、RocketMQ 512分区推导 |
| **库表设计** | 5张核心表（榜单配置/用户得分/得分流水/历史快照/反作弊封禁），含分库分表策略与乐观锁版本号设计 |
| **整体架构图** | Mermaid flowchart + 分层文字描述（4层：接入/应用/中间件/存储，含 Redis 三集群物理隔离说明） |
| **核心流程** | 幂等上报（SETNX + Pipeline 多榜写入）、ZREVRANK 附近排名查询（完整 Go 代码）、好友榜分批 ZUNIONSTORE 异步预计算、双 ZSet + RENAME 原子快照 |
| **缓存架构** | 三级缓存（本地60% → Redis 99.9%）、ZSet 分层设计（只维护 Top-10万）、分片 ZSet + 500ms 后台归并、缓存一致性单向数据流 |
| **MQ 设计** | 5个 Topic 分区数推导（含得分顺序性保障：同 uid+board 路由同分区，防乱序写入 score_after） |
| **核心关注点** | 热点分桶 ZSet 方案（300万 ZADD/s 分散到64分片）、防刷分多层（L1限流→L2业务→L3 Flink实时流→L4展示过滤）、并列排名时间戳编码、大 Key 治理 |
| **容错性设计** | 分层限流/熔断/三级降级/ETCD 开关矩阵/兜底方案（含 Redis 全挂后 DB COUNT 估算降级路径） |
| **可扩展性** | Redis 在线 reshard 大 Key 迁移注意事项、多维度榜单无缝扩展（区县级）、活动榜弹性创建/销毁、DB 双写迁移方案 |
| **高可用运维** | 监控指标三类（数据一致性/性能/业务）、大型活动运维 checklist（活动前72h/前24h/活动中/活动结束全流程） |
| **面试高频10题** | Q1~Q10 均为排行榜场景定制（ZSet 大 Key P99 劣化原因/好友榜 ZUNIONSTORE 耗时量化/Redis-DB 不一致修复/非 Top-N 用户排名估算/Redis 幂等Key宕机容错/快照精确时刻/日周月多维度方案选型/黑产刷榜群体分析/MQ 堆积时历史记录一致性/附近排名隐藏陷阱） |
