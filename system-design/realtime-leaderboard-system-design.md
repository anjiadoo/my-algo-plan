...
- **禁止好友榜无缓存直查**：5000 个好友逐个 ZSCORE + 排序虽轻量，但无缓存时高频访问仍会给 Redis 带来不必要压力，须加缓存层
...
| Redis 热数据 | 见下方拆解 | **≈ 100 GB** | 全部活跃榜单 ZSet 数据 |
...
- **好友榜缓存**（Pipeline ZSCORE 结果序列化，仅缓存高频访问用户）：预计50万用户有缓存 × 平均结果2KB ÷ 1024² ≈ **1 GB**
- **辅助数据（Hash、Set、限流计数器等）**：约 **10 GB**
- **基础热数据合计**：1.4 + 4.8 + 4.2 + 28 + 1 + 10 ≈ 49.4 GB
- **加主从复制缓冲（10%）+ 内存碎片（15%）+ 扩容余量（20%）**：49.4 × 1.45 ≈ 72 GB，保守规划 **100 GB**
...
| 榜单聚合服务 | 3000（好友榜Pipeline取分+聚合，链路轻） | 50万 | **取250台** |
...
| **FriendBoardView** 好友榜视图 | 用户视角的好友榜缓存结果 | 用户ID、好友ID列表、缓存的好友排名结果、缓存过期时间 | Pipeline ZSCORE 批量取好友分数 + 内存排序 + 结果缓存 | 最终一致，TTL 5~30s |
...
- **榜单聚合服务**：好友榜 Pipeline ZSCORE 批量取分 + 内存排序 + 结果缓存、Top-N 分页、附近排名聚合

**三、中间件层**：Redis 按榜类型物理隔离（全局榜/好友榜缓存/限流），MQ 承载异步落 DB 和跨服务解耦，ETCD 动态下发配置开关。
...
**好友榜定义**：在同一个榜单（如排位赛全局榜）中，只展示"我和我的好友"的排名子集。本质是从全局榜 ZSet 中批量查询好友分数，再排序展示。

**核心方案：Pipeline ZSCORE + 内存排序 + 多级缓存**

```
数据模型：
  - 全局榜 ZSet：board:{board_id}，member=uid，score=分数（已存在，无需额外数据结构）
  - 好友关系：由社交服务提供好友UID列表
  - 好友榜结果缓存：friend_board:{uid}:{board_id}，String（序列化JSON），TTL=30s

查询流程：
  1. 获取用户好友列表（最多5000人）
  2. Pipeline 批量 ZSCORE 从全局榜取所有好友的分数（1次RTT）
  3. 加上用户自己的分数
  4. 内存排序，取 Top-N 返回
  5. 结果写入缓存（30s TTL）
```
...
```go
func GetFriendBoard(ctx context.Context, uid int64, boardID string, topN int) ([]RankEntry, error) {
    cacheKey := fmt.Sprintf("friend_board:%d:%s", uid, boardID)

    // Step1: 查本地缓存（5s TTL，拦截同一实例高频请求）
    ...
    // Step3: 缓存未命中，实时计算好友榜
    friends := getFriendList(uid) // 从社交服务获取好友UID列表（最多5000）
    friends = append(friends, uid) // 加上自己

    // Step4: Pipeline 批量 ZSCORE（1次RTT取所有好友分数）
    key := boardKey(boardID)
    pipe := rdb.Pipeline()
    cmds := make([]*redis.FloatCmd, len(friends))
    for i, fid := range friends {
        cmds[i] = pipe.ZScore(ctx, key, fmt.Sprint(fid))
    }
    pipe.Exec(ctx) // 5000次 ZSCORE 合并为1次网络请求，耗时 < 5ms

    // Step5: 收集结果，过滤未上榜好友（ZSCORE 返回 nil）
    entries := make([]RankEntry, 0, len(friends))
    ...
    // Step6: 内存排序（O(K×logK)，K最多5000，< 1ms）
    ...
    // Step8: 计算好友内排名
    for i := range entries {
        entries[i].FriendRank = i + 1
    }

    // Step9: 写缓存
    data := serialize(entries)
    rdb.Set(ctx, cacheKey, data, 30*time.Second)
    localCache.Set(cacheKey, entries, 5*time.Second)
...
```
Pipeline ZSCORE 5000次：合并为1次 RTT，Redis 内部逐条 O(1) 哈希查找，总计 < 5ms
内存排序 5000个元素：O(5000 × log5000) ≈ O(5000 × 13) ≈ 6.5万次比较，< 1ms
总耗时：约 5~8ms（满足 P99 < 10ms 目标）

对比 ZUNIONSTORE 方案：
  - 无需为每个好友维护独立 ZSet
  - 无需合并操作，直接从全局榜取分
  - 复杂度从 O(N×logK)（N=总元素数）降至 O(K)（K=好友数=5000）
```
...
策略：分批 Pipeline + 并发
  - 将5000好友分为5批 × 1000人/批
  - 每批 Pipeline ZSCORE 1000次（1次 RTT，< 2ms）
  - 5批并发执行，总耗时 ≈ 单批最慢时间 ≈ 2~3ms
  - 合并结果后内存排序

限流保护：
  - 好友数 > 5000（超出上限）：只取最近互动的前5000好友
  - 缓存TTL根据好友数动态调整：好友数>2000 → TTL=60s，≤500 → TTL=15s
```
...
L2 Redis 集群（分布式，毫秒级）：
   ...
   ├── friend_board:{uid}:{board_id} String 好友榜缓存（序列化JSON，Pipeline ZSCORE结果排序后），TTL=30s
...
| `topic_friend_board_invalidate` | 5万条/s（得分变更时通知好友缓存失效） | **32** | 异步 | 好友榜缓存主动失效（可选优化） |
...
熔断后策略（分级）：
  ...
  - 好友榜：返回全局榜 Top10 代替（好友分数获取失败时降级）
...
lb.switch.friend_board: true        # 好友榜开关（故障时可关闭减少Redis读压力）
...
lb.friend_board.max_friends: 5000  # 好友榜最大查询好友数（超出截断取最近互动）
```
...
lb_friend_board_query_time          好友榜查询耗时P99（< 10ms）
```
...
| P2 | CPU>85%、内存>85%、好友榜缓存命中率<80% | 30min响应，告警通知 |
...
### Q2：好友榜需要查询5000个好友在同一榜单中的分数并排序展示。如果用户好友数极多（5000人），Pipeline ZSCORE 5000次会不会有性能问题？如何优化？

**参考答案：**

**先量化问题，再设计方案。**

**Pipeline ZSCORE 性能分析：**
- 好友数 K=5000，Pipeline 将5000次 ZSCORE 合并为1次网络 RTT
- Redis 内部逐条执行 ZSCORE：O(1) 哈希查找（ZSet 底层 hashtable），5000次操作 < 1ms
- 网络层：请求体 ≈ 5000 × 30B（命令+key+member）≈ 150KB，响应体 ≈ 5000 × 16B ≈ 80KB
- **总耗时约 3~8ms**（1次 RTT + Redis 内部执行 + 序列化），满足 P99 < 10ms 目标
- 内存排序：O(5000 × log5000) ≈ 6.5万次比较，< 1ms

**结论：5000次 Pipeline ZSCORE 在性能上完全可接受，无需异步预计算。**

**潜在问题与优化：**

① **网络包过大（好友数 > 5000 或 key 较长）**：
```
单个 Pipeline 请求体过大（> 1MB）会增加 Redis 网络缓冲区压力
解决：分批 Pipeline，每批1000人，5批并发执行
  5批 × 1000人/批 × 并发，耗时 ≈ 单批最慢 ≈ 2~3ms
```

② **Redis 单线程阻塞风险**：
```
5000次 ZSCORE 虽然每次 O(1)，但连续执行占用 Redis 单线程约 0.5ms
在 300万 QPS 的写入压力下，这 0.5ms 的阻塞可接受
若好友榜查询 QPS 过高（> 10万），批量 ZSCORE 累积可能影响其他命令
解决：好友榜读请求路由到从库（读写分离），不影响主库写入性能
```

③ **缓存策略（减少重复计算）**：
```
多级缓存：
  L1 本地缓存（5s TTL）：同一用户短时间多次查看不重复请求 Redis
  L2 Redis String 缓存（30s TTL）：序列化排序结果，跨实例共享
  
主动失效（可选优化）：
  好友得分变更时，通过 MQ 通知删除相关用户的好友榜缓存
  friend_board_invalidate:{uid}:{board_id} → DEL
  权衡：增加写放大（每次得分变更通知 K 个粉丝），适合好友数少的场景
  
被动失效（推荐）：
  TTL 到期自然过期，30s 内好友排名变化对用户感知影响极小
  简单可靠，无额外写放大
```
...
```go
// 用户想知道"我在好友中排第几"
// 方案1：从缓存的好友榜结果中二分查找自己的位置，O(logK)
// 方案2：ZSCORE 取自己分数，与好友分数比较，COUNT(score > myScore) + 1
// 推荐方案1，因为好友榜结果已缓存
```
...
> 2. **好友榜的 Pipeline ZSCORE + 缓存** 是轻量高效的业界标准方案——从全局榜批量取好友分数 + 内存排序，无需额外数据结构；
