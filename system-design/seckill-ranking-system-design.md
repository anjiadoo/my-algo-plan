...
| 决策 | 选择 | 核心理由 |
|------|------|---------|
| **Redis 预扣库存 + Lua 原子脚本** | 秒杀开始前将库存加载到 Redis，抢购时 Lua 脚本原子执行"校验+扣减" | DB 直扣在 500万 QPS 下必死（行锁竞争），Redis 单线程 Lua 天然串行化，无需分布式锁 |
| **库存分桶（Segment Inventory）** | 单个热点 SKU 库存拆成 N 个桶，每桶独立 Redis Key，uid 哈希路由 | 单 Key 10万 QPS 上限无法承载百万级热点，分 100 桶后单桶仅 1万 QPS |
| **异步下单 + 令牌机制** | Redis 扣库存成功后发放"下单令牌"（JWT），用户凭令牌异步创建订单 | 秒杀瞬间只做库存判定（< 5ms），重逻辑的订单创建走 MQ 异步（< 200ms），分离热路径与冷路径 |
| **排行榜 Redis ZSet + 分桶聚合** | 按品类维护 ZSet（score=销量），单 ZSet 上限 10万商品；超大品类按哈希分桶，定时合并 TopN | ZSet 的 ZINCRBY O(logN) + ZREVRANGE O(logN+K) 天然适配实时排行，亿级商品通过分桶控制单 ZSet 规模 |
| **排行榜与下单链路完全解耦** | 下单成功后发 MQ 事件，排行榜服务独立消费更新 ZSet，排行榜故障不影响下单 | 排行榜是展示型功能（允许秒级延迟），绝不允许阻塞核心交易链路 |
| **订单号全局唯一 + 幂等令牌** | 订单号 = 时间戳(ms) + 机器ID + 序列号（雪花变体），下单接口按 token 幂等 | 网络抖动/用户重试场景下，同一令牌只创建一笔订单，杜绝重复扣款 |
| **15分钟未支付自动释放库存** | 延迟消息（RocketMQ SCHEDULE）15分钟后检查订单状态，未支付则回滚 Redis + DB 库存 | 防止恶意占库存（"锁单攻击"），释放后其他用户可继续购买 |
| **排行榜三层架构（实时/小时/日榜）** | 实时榜 Redis ZSet（秒级更新）、小时榜定时快照（Spark 聚合写 Redis）、日榜持久化 MySQL | 不同时间粒度的排行榜查询需求不同，分层降低实时计算压力 |
| **支付回调幂等 + 对账** | 支付回调接口按 payment_id 幂等，日级全量对账：已支付订单金额 = 支付渠道流水金额 | 支付渠道可能重复回调（至少一次语义），幂等 + 对账双保险 |
| **下单 MQ 削峰控制 DB 写入速率** | 秒杀瞬时 50万笔/s 扣库存成功，经 MQ 削峰后 DB 订单写入受控在 3万 TPS | DB 直写 50万 TPS 需 167 个主库；MQ 削峰后仅需 10 个主库，节约 16 倍成本 |
