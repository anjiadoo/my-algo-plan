...
| # | 决策 | 选择 | 核心理由 |
|---|------|------|---------|
| 1 | **音视频云服务** | 腾讯TRTC作为底层音视频基础设施 | TRTC提供全球接入网络+自研私有UDP协议+端到端<300ms延迟；统一覆盖直播推流/拉流/连麦/PK场景，避免自建媒体服务器的巨大成本 |
| 2 | **推流方案** | TRTC SDK推流（主）+ RTMP推流（OBS/三方工具兼容） | TRTC SDK推流可享受私有UDP弱网对抗能力（30%丢包仍流畅）；兼容RTMP推流满足OBS等第三方工具需求 |
| 3 | **拉流方案** | TRTC低延迟拉流（互动场景）+ CDN旁路直播（HTTP-FLV/HLS大规模分发） | TRTC拉流延迟<1s适合互动直播；CDN旁路直播支撑百万级观众，TRTC云端自动旁路推流到CDN |
| 4 | **长连接网关** | 自研WS长连接网关，单机5万连接，按房间分片 | 信令（进房/退房/在线列表/系统消息）走长连接实时推送；音视频流走TRTC/CDN，两条链路物理隔离 |
| 5 | **房间状态管理** | Redis Hash存房间热数据 + MySQL持久化房间元数据 | 房间在线数/状态/连麦席位等高频读写走Redis（<1ms）；房间配置/历史记录走MySQL（强一致） |
| 6 | **在线观众管理** | 分层计数：精确小集合（<1万用Redis Set）+ 近似大集合（>1万用HyperLogLog + 计数器） | 小房间需精确在线列表（展示观众头像）；大房间只需近似人数（精确维护500万Set代价过高） |
| 7 | **连麦架构** | TRTC低延迟通信（主播+连麦者进同一TRTC房间）+ 云端混流 + CDN旁路直播 | TRTC房间内延迟<300ms满足实时互动；云端混流MCU自动合屏；旁路推流到CDN给普通观众（1-3s延迟可接受） |
| 8 | **PK机制** | TRTC跨房连麦(connectOtherRoom) + 云端混流合屏 + 独立PK计分服务 | TRTC原生支持跨房间音视频互通；云端混流自动合屏；PK业务逻辑（计分/惩罚）由自研服务管理 |
| 9 | **流状态机** | 推流状态机：idle→pushing→live→closing→idle，状态变更通过MQ广播 | 状态机保证开播/关播/断线重连等场景的一致性；MQ广播让所有依赖方（房间服务/推荐）异步感知状态变更 |
| 10 | **CDN调度** | TRTC旁路推流自动分发到腾讯云CDN + 备用CDN互备 + 故障自动切换 | TRTC云端混流后自动旁路推流到CDN；多CDN故障时3s内切换；HTTPDNS精确调度 |
...
| **进房** | 进房鉴权、建立长连接、获取房间快照 | 观众进入→鉴权→建WS长连接→下发房间信息（在线数/主播信息/流地址） |
...
| 维度 | 指标 |
|------|------|
| 可用性 | 推流链路 99.99%，拉流链路 99.99%，信令链路 99.9% |
| 性能 | 首屏 P95 < 1s，进房 P99 < 500ms，连麦建连 P99 < 1s |
| 延迟 | 标准直播端到端 1-3s，连麦/PK端到端 < 300ms |
| 峰值 | 进/退房信令 **100万 QPS**，拉流调度请求 **500万 QPS**，长连接 **5000万** |
...
| 信令入口 | 100万QPS × 1KB | **8 Gbps** |
| 信令出口（推送） | 5000万连接 × 10条/s × 200B | **80 Gbps** |
| 连麦实时传输 | 100万连麦中 × 1Mbps × 2(上下行) | **2 Tbps**（TRTC云端承载） |

> 说明：推流/拉流/连麦的音视频带宽全部由TRTC+CDN云服务承载，业务自身主要承担信令推送的80Gbps出口带宽。这是使用TRTC的核心收益——将最重的音视频带宽压力转移给云服务。
...
| topic_room_signal | 200万/s | 300B | 进房/退房/在线数变更信令 |
| topic_interact | 100万/s | 400B | 连麦/PK信令 |
...
| 服务 | 单机能力 | 峰值负载 | 机器数 | 说明 |
|------|---------|---------|-------|------|
| WS长连接网关 | 5万连接/机 | 5000万连接 | **1200台** + 400热备 | 16核32G，epoll+goroutine |
...
| Job路由层 | 10万条/s | 500万条/s | **80台** | MQ消费+gRPC分发 |
...
| **ViewerJoined** | 观众进房 | 用户ID、房间ID、进房时间、来源渠道 | WS连接建立+进房信令 | ①在线数+1 ②在线列表更新 ③欢迎消息广播(可选) |
| **ViewerLeft** | 观众退房 | 用户ID、房间ID、退房时间、观看时长 | WS断开/退房信令 | ①在线数-1 ②在线列表更新 ③观看记录落盘 |
...
│  │  └────────┘  └────────┘  └────────┘  └────────┘  └────────────────┘ │   │
│  └──────────────────────────────────────────────────────────────────────┘   │
│     │信令(WS)  │音视频(TRTC私有UDP)  │RTMP(兼容)   │拉流(CDN HTTP-FLV/HLS) │
│     ▼          ▼                     ▼             ▼                       │
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │                        接入层                                         │   │
│  │  ┌─────────────┐  ┌────────────────────────────────┐                │   │
...
│  │         ▼              应用层                                         │   │
│  │  ┌───────────┐  ┌───────────┐  ┌───────────┐  ┌───────────┐         │   │
│  │  │  房间服务  │  │ 流管理服务 │  │拉流调度服务│  │连麦信令服务│         │   │
│  │  │(进退房/    │  │(开播/关播/ │  │(CDN选择/  │  │(邀请/应答/│         │   │
│  │  │ 状态管理)  │  │ TRTC回调)  │  │ 地址生成) │  │ 席位/TRTC)│         │   │
│  │  └───────────┘  └───────────┘  └───────────┘  └───────────┘         │   │
│  │                                                                       │   │
│  │  ┌───────────┐  ┌───────────┐  ┌───────────┐  ┌───────────┐         │   │
│  │  │ PK服务    │  │在线观众服务│  │Job路由层   │  │UserSig服务│         │   │
│  │  │(匹配/计分/│  │(计数/列表/ │  │(MQ消费/   │  │(TRTC鉴权/ │         │   │
│  │  │ 结算)     │  │ 分层管理)  │  │ gRPC分发) │  │ 签名生成) │         │   │
│  │  └───────────┘  └───────────┘  └───────────┘  └───────────┘         │   │
│  └───────────────────────────────────────────────────────────────────────┘   │
│                              │                                               │
...
1. **音视频流与信令物理隔离**：音视频走TRTC云端+CDN（带宽密集型），信令走WS长连接网关（连接密集型），互不干扰
2. **TRTC统一音视频底座**：推流/拉流/连麦/PK/混流/旁路CDN/录制全部由TRTC云端处理，业务层只需管理信令和业务逻辑
3. **房间状态事件驱动**：所有状态变更（开播/关播/进退房/连麦/PK）通过MQ事件广播，下游服务自治消费
4. **大小房间差异化策略**：小房间（<1万人）走精确管理（完整在线列表），大房间（>1万人）走近似+抽样
5. **TRTC房间与业务房间映射**：业务room_id与TRTC trtc_room_id一一映射，连麦/PK场景通过TRTC跨房间能力实现
...
│                                                                         │
│  ③ 路径B：建立WS长连接（有状态，信令通道）                                 │
│     ┌─────────────────────────────────────────────────────────┐         │
│     │ WS网关处理流程：                                           │         │
│     │ 1. Ticket鉴权（JWT验签，本地公钥校验，<0.1ms）             │         │
│     │    Ticket由业务网关签发，含user_id/room_id/expire_ts       │         │
...
│     3. 通知房间服务：ViewerLeft(user_id, room_id)                         │
│         │                                                               │
│         ▼                                                               │
│  ③ 房间服务处理退房                                                       │
│     1. 更新在线计数：                                                    │
...
│     ┌─────────────────────────────────────────────────────────┐         │
│     │ 场景1：主播邀请观众上麦                                     │         │
│     │   主播 → WS信令 → 连麦信令服务 → 检查席位空闲              │         │
│     │   → 向目标观众推送邀请 → 观众接受/拒绝                      │         │
│     │                                                          │         │
│     │ 场景2：观众申请上麦                                        │         │
│     │   观众 → WS信令 → 连麦信令服务 → 加入申请队列              │         │
│     │   → 主播收到申请列表 → 主播同意/拒绝                        │         │
│     │                                                          │         │
│     │ 场景3：自由上麦模式                                        │         │
│     │   观众 → WS信令 → 连麦信令服务 → 检查席位空闲              │         │
│     │   → 直接占位上麦（无需主播确认）                            │         │
│     └─────────────────────────────────────────────────────────┘         │
│         │ 双方同意连麦                                                   │
│         ▼                                                               │
...
│  ├── DON'T：业务逻辑、数据库操作、复杂计算                                │
│  └── 原则：网关层只做I/O复用，业务处理交下游服务                           │
│                                                                         │
│  ┌─────────────────────────────────────────────────────────┐            │
│  │                  单节点内部架构                            │            │
│  │                                                          │            │
...
| 信令链路 | WS网关节点宕机 | 客户端自动重连到其他节点 | < 5s |
| 信令链路 | Job路由层故障 | MQ自动Rebalance到存活实例 | < 20s |
...
│                                                              │
│  1. 分离关注点：                                              │
│     音视频流(TRTC+CDN) ≠ 实时信令(WS) ≠ 业务逻辑(微服务)      │
│     三者物理隔离，各自独立扩展                                  │
│                                                              │
│  2. 云服务托管重资产：                                         │
...
| 信令 | WebSocket长连接 | 双向实时通信（进退房/弹幕/礼物/系统消息） |
