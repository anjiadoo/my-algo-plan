...
| 章节 | 核心内容 |
|------|---------|
| **需求与边界** | 功能边界（发/抢/查/退款/历史）、明确禁行（禁止超抢、禁止实时拆包、禁止 DB 直连抢包链路） |
| **容量评估** | 春节 200万 QPS 闭环验证、带宽/存储逐步推导、Redis 热数据 50GB 详细拆解（含单红包 6 个 Key 内存明细）、DB 分库数推导、RocketMQ 8主8从推导 |
| **库表设计** | 按"实体+事件+读模型+补偿设施"四类建模（实体 RedPacket 含 Slots 子项/GrabRecord、事件 PacketSent/Grabbed/Expired/RefundCompleted、读模型 AccountFlow/PacketDetailView/UserPacketHistory、补偿设施 SendTransaction/RefundTask）、6张核心表完整 SQL（红包主表/抢包记录/槽位表/资金流水/退款任务/事务补偿） |
| **整体架构图** | Mermaid flowchart + 四层文字描述（接入/应用/中间件/存储），核心设计原则三条 |
| **核心流程** | 二倍均值法拆包（含 Go 代码）、发包完整流程、抢包 Lua 脚本（6步原子操作）+ 三级缓存链路（含 Go 代码）、过期退款流程 |
| **缓存架构** | 三级缓存（go-cache/Redis/MySQL）、Redis Key 内存估算表、缓存穿透/击穿/雪崩/主动失效全覆盖 |
| **MQ 设计** | 5个 Topic 分区数完整推导（含峰值速率来源说明）、金融级三重可靠性保障、堆积处理方案 |
| **核心关注点** | 防超抢三层闭环（Redis List天然上限/Lua原子/DB唯一索引）、防重抢四道拦截、金额精度保障、超级热点分片 List 方案（含 Go 代码） |
| **容错性设计** | 分层限流/熔断阈值/三级降级/ETCD 动态开关/五类故障兜底矩阵 |
| **可扩展性** | K8s HPA、Redis 在线 reshard、DB 一致性哈希双写扩容、三级冷热分层存储 |
| **高可用运维** | 五类核心监控指标（金融安全/性能/业务）、三级告警阈值、春节四阶段运维 checklist |
| **面试高频10题** | Q1~Q10 均为红包场景定制（主从切换超抢/OOM中断发包/DB异步落库/通知风暴/状态判断/跨库查询/手气最佳/金融等式/空弹处理/企业红包大批量写入） |
