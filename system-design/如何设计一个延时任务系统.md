...
- **禁止在线扩 RocketMQ 队列**：扩队列会破坏同 task_id 同队列的保序保证，导致 ABA 问题
...
- DB 写入来源明细：
  ...
  |---------|---------|-----------|---------|
  | 任务提交 INSERT | 50万 | 10万（RocketMQ 削峰） | 1562 |
...
- 保留 3 天 = **65 TB**
- 队列数：**256 个 MessageQueue**（一次性预留，禁止在线扩队列；当前实际 50万 QPS / 256 = 1953 QPS/队列，安全）
- Broker：8 Broker 节点（DLedger 主从自动切换 × 2机房），16核32G + NVMe SSD
- 原生能力：延迟消息（trigger_retry 梯度重试）+ 死信队列（trigger_dead_letter）
...
> 说明：延时任务系统是典型的"写入削峰 + 冷热分层调度 + 异步回调"场景——写入链路写 RocketMQ + Redis meta 即返回，调度链路由分桶 owner 独占扫描时间轮触发，触发链路通过 RocketMQ 解耦回调投递。因此不按 DDD 聚合组织（Task/ExecLog/DeadLetter 不在同一事务边界内），而是按"实体（Entity）/ 事件（Event）/ 辅助实体（Auxiliary）/ 运维视图（Read Model）"四类梳理，更贴近真实架构职责。
...
**② 事件（Event，RocketMQ 事件流，不可变）**

| 模型 | 职责 | 核心属性 | 触发时机 | 对应 Topic | 下游消费 |
|------|------|---------|---------|-----------|---------|
| **TaskSubmitted** 提交事件 | 业务方提交任务，作为写入削峰的入口 | task_id、tenant_id、biz_type、biz_id、trigger_time、callback、payload | API Service 接收请求后异步发送 | `task_submit`（128队列） | Loader：写 DB + 写 Redis ZSet |
| **TaskDispatched** 触发投递事件 | 时间轮到期后投递给 Dispatcher 执行回调 | task_id、trigger_time、owner_epoch、payload | Scheduler 时间轮 onFire + WAL 落盘后 | `trigger_dispatch`（256队列） | Dispatcher：执行 HTTP/RPC/MQ 回调 |
| **TaskRetried** 重试事件 | 回调失败后利用 RocketMQ 原生延迟消息进入梯度重试 | task_id、retry_cnt、next_retry_time、last_error | Dispatcher 回调失败时 | `trigger_retry`（64队列，延迟消息） | Dispatcher：延迟后重新投递（1s/5s/30s/5m/1h） |
| **TaskDead** 死信事件 | 重试耗尽，进入 RocketMQ 原生死信队列 | task_id、tenant_id、biz_type、biz_id、last_error、retry_cnt | 第 N 次重试仍失败（N=max_retry） | `%DLQ%trigger_retry`（原生死信） | 告警服务 + 人工介入 |

> 事件是**追加写、不可变的事实记录**，通过 RocketMQ 解耦写入/调度/触发三条链路。同一 task_id 的所有事件路由到同一 MessageQueue（sharding key=task_id），保证有序性——避免"取消消息比触发消息先到达"的 ABA 问题。RocketMQ 原生延迟消息天然支持重试梯度，死信队列自动托管重试耗尽的消息。

**③ 辅助实体（Auxiliary，非核心写路径，异步产生）**

| 模型 | 职责 | 核心属性 | 生成方式 | 存储位置 |
|------|------|---------|---------|---------|
| **TaskExecLog** 执行流水 | 记录每次触发尝试的结果，用于问题排查和延迟分析 | task_id、执行时间、期望触发时间、结果（成功/失败/取消/重试）、耗时ms、错误信息 | Dispatcher 回调完成后异步写入 | ClickHouse `task_exec_log`（保留30天，列式存储适合高频追加写+分析查询） |
...
└──────┬───────┘                                    └──────────────────┘
       ...
       └────────→     │ TaskSubmitted  │                     │
                      ...
                      └───────┬────────┘                     │
                              ...
                              ↓                              │
                      ┌────────────────┐                     │
                      │ TaskDispatched │──RocketMQ──→ Dispatcher
                      │(trigger_dispatch)│           │       │
                      └────────────────┘             │       │
                                                     ↓       │
                                                 回调业务方 ───┘
...
- **写入链路极致轻量**：API 写 RocketMQ + Redis meta 即返回（< 5ms），不阻塞触发链路
- **DB 是唯一权威源**：Redis/时间轮/RocketMQ 都是加速层，丢失可从 DB 重建
- **状态机单向不可逆**：PENDING→LOADED→TRIGGERED→ACKED，任何逆向操作都是"墓碑+新建"而非原地修改
- **事件流是系统骨架**：四个 RocketMQ Topic 对应四个关键事件，串联写入/调度/触发三条链路
...
-- 执行流水（仅记录成功/失败，不记每次重试；存储在 ClickHouse，适合高频追加写+延迟分析）
-- ClickHouse DDL
CREATE TABLE task_exec_log (
  task_id      UInt64,
  exec_time    DateTime64(3),
  trigger_time DateTime64(3) COMMENT '期望触发时间（用于计算延迟抖动）',
  tenant_id    UInt32,
  biz_type     LowCardinality(String),
  result       UInt8 COMMENT '0 成功 / 1 失败 / 2 取消 / 3 重试',
  cost_ms      UInt32,
  error_msg    String,
  INDEX idx_task task_id TYPE minmax GRANULARITY 4
) ENGINE = MergeTree()
PARTITION BY toYYYYMMDD(exec_time)
ORDER BY (tenant_id, task_id, exec_time)
TTL exec_time + INTERVAL 30 DAY
COMMENT '任务执行流水（ClickHouse 列式存储，保留30天，支持高吞吐写入+延迟抖动分析）';
...
### 3.4 RocketMQ Topic 设计

| Topic | 队列数 | key 路由 | 消费者 | 用途 | 保留 |
|-------|--------|----------|--------|------|------|
| `task_submit` | 128 | task_id | Loader | 写入削峰，API→Loader | 3天 |
| `trigger_dispatch` | 256 | task_id | Dispatcher | 时间轮→回调投递 | 3天 |
| `trigger_retry` | 64 | task_id | Dispatcher | 失败重试（原生延迟消息，5级梯度：1s/5s/30s/5m/1h） | 3天 |
| `trigger_dead_letter` | 16 | task_id | 无消费者 | 人工介入查询（RocketMQ 原生死信队列） | 30天 |

> **队列数一次性预留不做在线扩队列**：扩队列会改变 hash(task_id) % N 的路由，破坏同 task_id 同队列的保序保证。预留 256 队列足以支撑 256×4000=100万 QPS（2倍峰值）。RocketMQ 原生延迟消息天然支持 trigger_retry 的梯度重试，无需额外延迟实现。
...
flowchart TD
    ...
    subgraph 应用层
        API["DelayTask API Service<br/>提交/取消/修改/查询<br/>同步写 Redis meta + 异步写 RocketMQ"]
    ...
    subgraph 中间件层
        ...
        RMQ["RocketMQ 集群<br/>跨可用区 / 原生延迟消息<br/>task_submit/trigger_dispatch/retry/dead_letter"]
        ETCD["ETCD 集群<br/>Raft 选主存储 / 动态配置<br/>bucket_owner / 限流阈值 / 开关"]
    end

    subgraph 存储层
        DB["MySQL 集群<br/>64主128从 / 按 bucket_id%64 分库<br/>按 trigger_day 分7张日表滚动"]
        CK["ClickHouse 集群<br/>任务执行流水 task_exec_log<br/>列式存储 / 延迟分析 / 保留30天"]
...
- **写入链路**（API Service）：同步写 Redis meta（保证 Dispatcher 可查）+ 异步写 RocketMQ（削峰入库），5ms 内返回
- **调度链路**（Scheduler）：分桶 owner 独占扫描 Redis ZSet，时间轮触发，写 RocketMQ trigger_dispatch
- **触发链路**（Dispatcher）：消费 RocketMQ 执行回调，支持 HTTP/RPC/MQ 三协议，失败进重试队列
...
2. API Service 处理：
   ...
   g. 异步写 RocketMQ task_submit（key=task_id，保证同任务同队列）→ 5ms 内返回 task_id
3. Loader 消费 RocketMQ：
...
**为什么 API 同步写 Redis meta + 异步写 RocketMQ：**
- Redis meta 同步写保证 Dispatcher 消费时能立即查到 callback/payload，不会出现"触发了但查不到 meta"
- RocketMQ 异步削峰保证 DB 不被打爆（50万 QPS 直写 DB 需 100+ 主库）
- 两者组合：API 返回延迟 ≈ Redis RTT(2ms) + RocketMQ Produce(3ms) ≈ **5ms**

**写入降级（RocketMQ 不可用时）：**
- API 检测 RocketMQ 超时 → 降级为同步写 DB + Redis ZSet
- 延迟升高到 10~20ms，但不拒绝请求
- 告警触发后 ops 介入恢复 RocketMQ
...
func (s *Scheduler) onFire(taskID int64) {
    ...
    // Step 3: 投递到 RocketMQ trigger_dispatch
    err := rocketmq.SendSync("trigger_dispatch", taskID, payload)
    if err != nil {
        // RocketMQ 失败：保留 WAL，定时任务扫描重投；不重试阻塞时间轮
        metrics.IncRocketMQFail()
        return
    }
...
**两阶段 ACK 防丢任务：**
- Phase 1：WAL 落盘 + RocketMQ 投递成功
- Phase 2：Dispatcher 回调成功 → RocketMQ ACK → DB status=ACKED
- Phase 1 完成但 Phase 2 未完成（进程 crash）：重启从 WAL 重放，Dispatcher 幂等处理

### 5.4 回调投递流程（触发链路）

```
Dispatcher 消费 RocketMQ trigger_dispatch：
...
- 任务可能已过期但 RocketMQ 消息在积压中（积压可能超过 1 小时）
- +7200（2小时）保证覆盖 RocketMQ 最大积压时间
- 自动过期释放内存，不占额外空间
```
...
2. Dispatcher 识别 FANOUT 类型，转发给 FanoutService（独立 RocketMQ Topic: fanout_dispatch）
3. FanoutService 处理：
...
| Topic | 队列数 | key 路由 | 消费者 | 用途 |
|-------|------|----------|--------|------|
| `task_submit` | 128 | task_id | Loader | 写入削峰，API→Loader |
| `trigger_dispatch` | 256 | task_id | Dispatcher | 时间轮→回调投递 |
| `trigger_retry` | 64 | task_id | Dispatcher | 失败重试（原生延迟消息，5级梯度） |
| `trigger_dead_letter` | 16 | task_id | 无消费者 | 人工介入查询（原生死信队列） |

**队列数推导（一次性预留，禁止在线扩队列）：**
- `trigger_dispatch`：50万 QPS / 2000 QPS 单队列安全上限 = 250 → 向上取整 **256**
- `task_submit`：50万 QPS 削峰后消费侧控制在 10万 QPS，10万 / 1000 = 100 → 取 **128**（预留扩展）
- **关键约束**：队列数确定后不再变更，避免 hash(task_id) % N 路由变化导致的保序破坏
- **RocketMQ 原生优势**：`trigger_retry` 直接利用 RocketMQ 延迟消息能力（支持任意延迟时间），无需自行实现延迟逻辑

### 7.2 可靠性三层保障

**生产者：**
- `SYNC_MASTER` + `SYNC_FLUSH`，确保消息同步写入主从 Broker 并刷盘
- `trigger_dispatch` 使用同步刷盘（SYNC_FLUSH）：丢消息 = 丢任务 = 资损
- `task_submit` 异步刷盘（ASYNC_FLUSH，性能优先，接入层写入量大）

**Broker：**
- 8 Broker × 主从模式（DLedger 自动选主，含 1 跨可用区副本）
- 单 Broker 宕机自动切主（DLedger Raft 机制）
- 保留 3 天，支持故障恢复回放

**消费者：**
- 手动提交 offset，业务处理成功后才 ACK
- 幂等消费：Dispatcher 依赖 DB `status` 判断是否已处理（status 是 LOADED→TRIGGERED→ACKED 单向流转）
- 消费失败不 ACK → RocketMQ 自动重试（利用原生延迟重投机制）

### 7.3 同任务保序

**问题：** 取消消息和触发消息如果分布在不同队列，可能乱序到达 Dispatcher。

**解决：** 所有该 task_id 相关消息都用 `task_id` 作为 sharding key，RocketMQ 顺序消息保证同 key 同 MessageQueue，队列内 FIFO：
```
队列 0: [submit(t1), update(t5), cancel(t8), trigger_dispatch(t10)]
                              └─> Dispatcher 消费到 trigger_dispatch 时，墓碑已就绪
```

**禁止在线扩队列的原因：**
扩队列后 `hash(task_id) % 128` 和 `hash(task_id) % 256` 路由到不同队列。已取消的任务的 cancel 消息在旧队列 Q7，但触发消息可能被路由到新队列 Q135。Dispatcher 消费 Q135 时尚未消费 Q7 的 cancel → 已取消的任务被错误触发。

### 7.4 消息堆积应急

**监控：**
- `trigger_dispatch` 队列 lag > 1万 → P1 告警
- 总 lag > 10万 → P0 告警（意味着可能丢失 SLA）

**应急（不扩队列）：**
1. 临时扩容 Dispatcher 节点（K8s HPA 2 min 内扩容 3 倍）
2. 降级：暂停非核心租户（priority>5）回调
3. 增加消费者并发（单队列内多线程消费，队列数不变）
...
| RocketMQ 投递延迟 | 5~50ms | 同步刷盘，跨机房 100ms 上限 |
...
- RocketMQ 堆积 > 1000 条 → 熔断降级，优先核心租户
...
**第三层：Dispatcher 消费端 RocketMQ 流控**
...
- 已进入 RocketMQ 的消息：Dispatcher 通过 DB status 判重
```
...
| Dispatcher | 单机回调 QPS | 3000 | RocketMQ consumer pause |
...
| RocketMQ 投递失败率 > 5% | 熔断 Scheduler 投递，WAL 暂存，1min 后半开重试 |
...
| RocketMQ 集群宕机 | API 降级同步写 DB+Redis；Scheduler WAL 本地暂存，限流到 1 万 QPS，人工介入 |
...
### 10.5 RocketMQ 容量扩展（不扩队列）

**队列数不变，吞吐不够时的扩展方式：**
- 增加 Broker 节点（MessageQueue 的 Leader 可在新 Broker 间重分配）
- 增加消费者实例（最多等于队列数 256 个）
- 启用消费者内部多线程（单 MessageQueue 拉取后多线程处理，保证 task_id 级串行）
...
- `rocketmq_consumer_lag` 按 Topic：消费堆积
...
- API 生成 traceID，写入 Redis meta + RocketMQ header
- Loader/Scheduler/Dispatcher 从 RocketMQ header 提取 traceID，继续传播
...
**大促中：**
- 禁止发布
- 实时 Dashboard 盯 P99 抖动、丢任务率、RocketMQ lag
...
- **RocketMQ 消息体加密**：payload 中若包含 PII（手机号/订单金额），使用 AES-256-GCM 加密存储，密钥按租户隔离（KMS 托管）
...
- 即便能删，RocketMQ trigger_dispatch 队列中可能已有该任务消息
...
**③ 墓碑 TTL 设计：**
TTL = max(trigger_time - now, 0) + 7200，保证覆盖 RocketMQ 最大积压时间（2h），即使任务已过期也有足够校验窗口。
...
Transaction {
  ...
  rocketmq.Send(...)  -- 不在事务内，crash 时状态不确定
  COMMIT
}
```
...
| 100ms | 全内存时间轮+独立 RocketMQ | 5x | 实时推荐 |
| 1s (本方案) | 冷热分层+RocketMQ | 1x | 订单/券/通知 |
| 10s | 纯 DB 轮询 | 0.3x | 统计任务 |

**② 1ms 精度的挑战：**
- RocketMQ 批量刷盘延迟就超过 1ms
- 跨机房网络往返 > 1ms
- 需要单机部署 + RDMA，不具备扩展性
...
4. **Tombstone 墓碑**：取消/修改的优雅处理，TTL 覆盖 RocketMQ 最大积压窗口
5. **RocketMQ 固定队列 + 同 key 同队列 + 原生延迟消息**：禁止在线扩队列，保证保序正确性，梯度重试零开发
