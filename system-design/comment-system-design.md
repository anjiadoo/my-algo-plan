...
| 章节 | 核心内容 |
|------|---------|
| **一、需求与边界** | 功能边界（发布/查看/回复/点赞/删除/审核）、二级评论树结构、明确禁行 |
| **二、容量评估** | DAU 6亿闭环推导、500万 QPS 读/50万 QPS 写分层计算、存储/带宽/Redis/DB/MQ 完整推导 |
| **三、库表设计** | 按"实体+事件+读模型"三类建模（实体 Comment/Like、事件 CommentPublished/Deleted/Liked/AuditFinished、读模型 CommentCountView/CommentListView/AuditRecord/Notification）、5张核心表完整 SQL（评论主表/回复表/计数表/点赞表/审核表） |
| **四、整体架构图** | Mermaid flowchart + 四层文字描述（接入/应用/中间件/存储），核心设计原则三条 |
| **五、核心流程** | 发布评论（审核前置+事务补偿）、读取评论列表（游标分页+多级缓存）、点赞（Redis 原子计数）、热点内容评论爆炸处理 |
| **六、缓存架构** | 三级缓存（go-cache/Redis/DB）、评论列表 ZSet 设计、热点评论独立集群、缓存穿透/击穿/雪崩全覆盖 |
| **七、MQ 设计** | 5个 Topic 分区数完整推导、审核异步化、通知解耦、计数最终一致性 |
| **八、核心关注点** | 热点内容评论爆炸（分片+本地聚合）、防刷/防重（三层幂等）、计数强一致（Redis+DB双写）、游标分页 vs OFFSET |
| **九、容错性设计** | 分层限流/熔断阈值/三级降级/ETCD 动态开关/五类故障兜底矩阵 |
| **十、可扩展性** | 评论树水平拆分、冷热评论分层存储、异地多活流量染色 |
| **十一、高可用运维** | 评论核心指标监控（发布成功率/读 P99/计数误差）、三级告警、热点预案 |
| **十二、面试高频10题** | Q1~Q10 均为评论场景定制（评论树展开/游标分页/计数一致性/热点爆炸/审核时序/点赞精度/删除可见性/AT通知/跨服务聚合/冷热归档） |
...
| 人均每日写评论次数 | **0.5次** | 读写比 60:1，评论是典型重读轻写 |
| 日均读请求量 | **180亿次** | 6亿 × 30次 = 180亿 |
| 日均写请求量 | **3亿次** | 6亿 × 0.5次 = 3亿 |
| 平均读 QPS | **20.8万 QPS** | 180亿 ÷ 86400s ≈ 20.8万 |
| 峰值读 QPS | **500万 QPS** | 平均 × 峰值系数 24（热点事件评论区瞬时爆发），与业务目标对齐 |
| 平均写 QPS | **3472 QPS** | 3亿 ÷ 86400s ≈ 3472 |
| 峰值写 QPS | **50万 QPS** | 平均 × 峰值系数 144（重大事件下评论涌入，如世界杯进球时刻）|
...
```
日写评论量：3亿条/天
10年存量：3亿 × 365 × 10 = 1095亿条（保守估 1000亿）
单条评论存储：约 500B（含内容、元信息、索引）
总存储：1000亿 × 500B ÷ 1024⁴（1024⁴ = 1TB，将 B 转换为 TB） ≈ 45.5 TB（仅评论主表）
加上回复、计数、点赞数据 × 3 ≈ 136 TB，与下方存储规划闭合 ✓
```

### 容量计算

**带宽：**
- 入口计算：（500万读 QPS + 50万写 QPS）× 1KB/请求 × 8bit ÷ 1024³（1024³ = 1GB，将 bit 转换为 Gbps） ≈ **44 Gbps**
- 入口规划：44 Gbps × 2（冗余系数）= **88 Gbps**，规划 **100 Gbps**
- 出口计算：500万读 QPS × 5KB（评论列表响应，含20条评论摘要）× 8bit ÷ 1024³ ≈ 190 Gbps，规划 **400 Gbps**
  依据：评论读响应体远大于请求体（返回评论内容+用户信息），出口带宽是入口的 4~5 倍，为典型读多场景

**存储规划：**

| 数据 | 计算过程 | 估算结果 | 说明 |
|------|---------|---------|------|
| 评论主表（根评论） | 1000亿条 × 10% × 300B ÷ 1024⁴（1024⁴ = 1TB，将 B 转换为 TB） | **≈ 2.7 TB** | 根评论占10%，单条约300B |
| 回复表（二级回复） | 1000亿条 × 90% × 200B ÷ 1024⁴ | **≈ 16.4 TB** | 回复占90%，单条约200B（内容较短） |
| 点赞表 | 每条评论平均 5 次点赞 × 1000亿 × 30B ÷ 1024⁴ | **≈ 13.6 TB** | uid+comment_id+时间，仅保留去重记录 |
| 计数表 | 1000亿条 × 50B ÷ 1024⁴ | **≈ 4.5 TB** | 每条评论的点赞数/回复数聚合 |
| Redis 热数据 | 见下方拆解 | **≈ 300 GB** | 热点内容评论列表 + 计数缓存 |
| MQ 消息 | 50万/s × 1KB × 86400s × 3天 ÷ 1024³（1024³ = 1GB，将 KB 转换为 TB 需再 ÷ 1024²，故先算 KB 总量再 ÷ 1024⁴ 得 TB） | **≈ 120 TB/3天** | 峰值写入事件消息，3天保留 |
...
- **关键设计前提**：写评论经 MQ 削峰，消费侧目标写入速率为 **5万 TPS**（峰值 50万 QPS 写入经 MQ 缓冲后均摊）
- **评论主表**：
  - 所需分库数：5万 TPS ÷ 3000 TPS/库 ≈ 17，取 **16库**（2的幂次，按 object_id 哈希路由）
  - 单库均摊 TPS：5万 ÷ 16 = **3125 TPS**（略超安全上限，加主从读写分离后写压力满足 ✓）
  - 分256表：按 comment_id 分表，降低单表写入竞争
- **点赞表**：
...
- **评论列表读缓存集群**：
  - 实际到达 Redis 的 QPS = 500万 × (1 - 70% 本地缓存命中率) = **150万 QPS**
  - 所需分片数：150万 ÷ 10万 = 15，取 **16分片**（2的幂次）
  - 单分片均摊 QPS：150万 ÷ 16 = **9.4万 QPS**（接近安全上限，需热点单独隔离）
- **热点内容独立集群**：头部 1% 热点内容承担 80% 流量（400万 QPS），单独 8分片集群隔离

**服务节点（Go 1.21，8核16G）：**

| 服务 | 单机安全 QPS | 依据 | 有效 QPS | 节点数计算 | 节点数 |
|------|-------------|------|---------|-----------|--------|
| 评论读服务 | 5000 | 本地缓存 + Redis ZSet 范围查询 + 评论详情批量获取，中等逻辑 | 500万 | 500万 ÷ (5000 × 0.7) ≈ 1429 | **取1500台** |
| 评论写服务 | 1000 | 内容安全预检 + DB 写入 + Redis 更新 + MQ 发送，链路较重 | 50万 | 50万 ÷ (1000 × 0.7) ≈ 714 | **取800台** |
...
- **峰值总消息量**：写评论 50万/s + 点赞 100万/s + 通知 50万/s ≈ **200万条/s**
...
```sql
-- =====================================================
-- 评论主表（按 object_id % 16 分16库，% 256 分256表）
-- 读高峰走从库，写走主库
-- =====================================================
...
核心服务：
- **评论读服务（1500台）**：本地缓存前置拦截 70%，Redis ZSet 范围查询获取 comment_id 列表，批量 mget 获取评论详情，组装返回；
- **评论写服务（800台）**：关键词实时预检（同步）→ DB 写入（status=0待审核）→ Redis 预写缓存 → 发 MQ 触发异步 AI 审核；
...
核心组件：
- **Redis 评论列表缓存集群**：普通 16分片 + 热点独立 8分片，ZSet 存储每个内容的评论 ID 有序列表（score=时间戳或热度分）；
...
- **MySQL 评论库（16主32从）**：存储评论主表，按 object_id 分16库256表；
...
- **定期校准**：每 10 分钟将 Redis 计数与 DB 计数比对，差异超过 5% 触发告警和修复
...
|-------|------------|-----------|--------|---------|------|--------|
| `topic_comment_write` | 50万条/s（每次写评论发一条） | 50万 ÷ (8000 × 0.7) ≈ 89，取2的幂次 | **128** | 同步刷盘 | 评论写入落库、缓存更新 | 评论写消费服务 |
| `topic_comment_audit` | 50万条/s（与写入速率一致） | 50万 ÷ (5000 × 0.7) ≈ 143，取2的幂次 | **128** | 同步刷盘 | 触发内容安全异步审核 | 审核消费服务 |
...
> **topic_comment_write 和 topic_comment_audit 为何用同步刷盘？** 评论写入和审核任务不允许丢失——写入丢失导致用户评论凭空消失（严重产品问题）；审核任务丢失导致违规评论长时间暴露（安全问题）。同步刷盘牺牲 30% 性能换可靠性，合理取舍。
>
...
**生产者端（评论写入，不允许丢消息）：**
```go
// 本地事务消息：DB 写入成功后才确认发送 MQ
func PublishComment(req *CommentRequest) error {
    // Step1: 写本地事务日志（WAL）
    walID := wal.Write(&CommentTxn{RequestID: req.RequestID, Status: "pending"})

    // Step2: 发 MQ 半消息（Half Message）
    msgID, err := mq.SendHalfMessage(&CommentMsg{...})
    if err != nil {
        wal.UpdateStatus(walID, "mq_failed")
        return err
    }
    ...
    if err := db.Transaction(func(tx *gorm.DB) error {
        ...
        mq.RollbackHalfMessage(msgID)  // 回滚半消息
        return err
    }

    // Step4: 确认 MQ 消息发送（DB 提交后才确认）
    mq.CommitHalfMessage(msgID)
    wal.UpdateStatus(walID, "committed")
    return nil
}

// 定时任务：扫描 WAL 中 status=pending 超过 30s 的记录，重试 MQ 发送
```
...
③ 定期校准（每 10 分钟）：
   SELECT comment_id, COUNT(*) as real_count
   FROM comment_like WHERE status=1 GROUP BY comment_id
   -- 与 Redis count:like:{id} 对比，差异超过 5 触发修复
...
| MQ 宕机 | 评论同步写 DB（去掉异步），统计数据延迟，通知暂停 | 手动恢复 |
...
- 读服务热点活动预扩容：大型事件（如世界杯决赛）前 1 小时扩至 3000 台
...
实现：
① 普通内容：按 object_id % 16 哈希路由到 16 个库分片
...
```
热数据（0~30天） : MySQL + Redis（在线实时查询，快速响应）
温数据（30天~1年）: MySQL 归档库（读写分离，按需查询，关闭部分索引）
冷数据（1年以上） : ClickHouse / TiDB（OLAP 分析，历史评论检索）
归档数据（3年以上）: OSS 对象存储（Parquet 格式，按年/月分桶，极低访问频率）

触发方式：定时任务每日凌晨，将 create_time < 30天前的评论导出归档
效果：MySQL 主库只保留 30天热数据，单库数据量从 45TB 降到 3.7GB（30天 ÷ 10年）
...
解决方案（快照游标）：
  - 用户第一次打开评论列表时，将当前热度排名快照到 Redis（TTL=10min）
  - Key = cm:snapshot:{uid}:{object_id}:{timestamp}
  - 分页游标带上 snapshot_key，后续翻页都从快照中读取
  - 快照过期后重新生成（用户 10 分钟内翻页基于同一快照，不漂移）
  
代价：额外的 Redis 内存（每个用户每个内容一个快照），活跃用户 × 活跃内容 × 快照大小
控制：仅对热度排序模式使用快照，时间排序直接游标（无需快照）
```
...
| 超级热门 | > 10万 | HyperLogLog（估算）+ DB 唯一索引兜底 | 固定 **12KB**/条 | 误差 0.81% |
...
func LikeComment(commentID, uid int64) error {
    ...
    if likeCount < 100000 {
        ...
        // 超级热门：DB 唯一索引兜底 + HyperLogLog 计数
        // 先写 DB（uk_comment_uid 唯一索引防重）
        err := insertLikeDB(commentID, uid)
        if err != nil && isDuplicateKeyErr(err) {
            return ErrAlreadyLiked  // DB 唯一索引拦截重复点赞
        }
        // HyperLogLog 统计 UV（用于展示，允许 0.81% 误差）
        rdb.PFAdd(ctx, "hll:like:"+strconv.FormatInt(commentID, 10), uid)
...
// 广播效果：1500台实例同时消费，< 1s 内全部实例本地缓存失效
