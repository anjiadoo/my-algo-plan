...
**MongoDB 评论集群（分片规划）：**
- **MongoDB 单分片安全写入上限**：约 5万~10万 TPS（WiredTiger 引擎，16核32G + NVMe SSD，批量写入场景可达 10万+ TPS）
- **关键设计前提**：写评论同步写 MongoDB（保证评论不丢），MQ 仅用于审核/通知/计数等下游异步任务。持续峰值 50万 QPS 由 MongoDB 分片集群直接承载；脉冲峰值 150万 QPS 由网关限流 + 应用层本地队列缓冲，平滑到 50万 QPS 后写入。
- **评论主集合**：
  - 所需分片数：50万 TPS ÷ 5万 TPS/分片 = 10，取 **16分片**（预留余量，按 object_id 哈希分片）
  - 单分片均摊 TPS：50万 ÷ 16 = **31250 TPS**（安全线内 ✓）
  - 分片策略：`sh.shardCollection("comment_db.comment", { object_id: "hashed" })`
  - 优势：同一 object_id 的所有评论在同一 chunk 内，列表查询不跨分片
- **点赞表（保留 MySQL）**：
...
|------|------|---------|---------|---------|
| **Comment** 评论 | 评论全生命周期：发布→审核→展示→删除（含二级回复） | 评论ID、被评论内容ID、内容类型、根评论ID、被回复评论ID、作者ID、评论内容、状态（0待审/1可见/2已删/3违规下架） | 发布、删除、状态变更、查询评论列表 | MongoDB 按 `object_id` 哈希分片（树形回复嵌套文档天然适配，字节跳动评论系统验证方案） |
...
```
  ...
  │  (MongoDB)   │──CommentDeleted────┼─MQ─→ 多订阅者 ─→│ (Redis权威)      │
  └──────┬───────┘                    │                 └──────────────────┘
         │                            │                 ┌──────────────────┐
...
**设计原则：**
- **写路径极简**：评论只写 MongoDB + 发一个 `CommentPublished` 事件，P99 < 200ms
- **计数不查 COUNT**：`CommentCountView` 是独立维护的 Redis 计数器，避免评论数一多就拖垮 DB
- **审核异步不阻塞发布**：先发后审（或先审后发的白名单），通过 `AuditFinished` 事件驱动状态变更
- **读模型可重建**：所有 ZSet/计数都可以通过回放事件流从 MongoDB 重新物化

### 3.2 完整存储设计

```javascript
// =====================================================
// 评论主集合（MongoDB，按 object_id 哈希分片）
// 业界最佳实践：字节跳动评论系统采用 MongoDB
// 理由：评论树形结构天然适配文档模型，无需递归 Join；
//       评论内容未来可扩展为富文本/图片/投票等，Schema 灵活
// =====================================================

// Collection: comment
// 分片策略: sh.shardCollection("comment_db.comment", { object_id: "hashed" })
// 同一内容的所有评论在同一分片内，评论列表查询不跨分片

// 文档结构（单条评论 = 一个文档）
{
  _id: ObjectId("..."),
  comment_id: NumberLong(xxx),        // 雪花算法ID，全局唯一
  object_id: NumberLong(xxx),         // 被评论内容ID（视频ID/帖子ID等）
  object_type: 1,                     // 1视频 2帖子 3文章 4动态
  root_id: NumberLong(0),             // 根评论ID，0=自身是根评论，非0=二级回复
  reply_to_id: NumberLong(0),         // 回复的具体评论ID（二级回复时有效）
  uid: NumberLong(xxx),               // 评论者用户ID
  content: "评论内容...",              // 评论内容
  // 文档模型优势：未来可扩展为富文本，无需 ALTER TABLE
  // rich_content: { type: "rich", blocks: [...] },  // 预留富文本扩展
  // media: [{ type: "image", url: "..." }],         // 预留图片评论扩展
  status: 1,                          // 0待审核 1审核通过 2审核拒绝 3用户删除 4管理员删除
  like_count: 0,                      // 点赞数（冗余，用于排序，Redis为权威）
  reply_count: 0,                     // 回复数（冗余，仅根评论有效）
  ip_region: "北京",                   // 发布时IP归属地
  create_time: ISODate("2026-01-15T10:30:00Z"),
  update_time: ISODate("2026-01-15T10:30:00Z")
}

// 索引设计
db.comment.createIndex({ object_id: 1, root_id: 1, create_time: -1 })  // 查某内容的根评论列表
db.comment.createIndex({ root_id: 1, create_time: 1 })                  // 查某根评论的回复列表
db.comment.createIndex({ uid: 1, create_time: -1 })                     // 查某用户发布的评论
db.comment.createIndex({ object_id: 1, status: 1, like_count: -1 })     // 热门评论排序

// 为什么选 MongoDB 而非 MySQL（业界最佳实践）：
// 1. 树形结构：评论-回复天然是树形/图结构，文档模型比邻接表/闭包表更直观
// 2. Schema 灵活：评论内容趋势是富文本化（图片评论、投票评论、表情包），文档模型无需 DDL
// 3. 读写模式匹配：评论以 object_id 为聚合根查询，MongoDB 分片后同内容评论在同分片内
// 4. 无复杂事务：评论发布是单文档操作，MongoDB 单文档原子性足够
// 5. 字节跳动验证：抖音/头条评论系统使用 MongoDB 承载千亿级评论
```

```sql
-- =====================================================
-- 内容评论计数表（MySQL，按 object_id % 4 分4库）
-- 记录每个内容的评论总数，写多读多，独立维护
-- 说明：计数表保留 MySQL 是因为 Redis INCR 为实时权威，MySQL 仅做对账兜底
-- =====================================================
...
-- =====================================================
-- 评论计数表（MySQL，按 comment_id % 8 分8库）
-- 记录每条评论的点赞数和回复数，高频更新
-- =====================================================
...
-- =====================================================
-- 点赞记录表（MySQL，按 comment_id % 8 分8库）
-- 核心：uk_comment_uid 防重复点赞（MySQL 唯一索引兜底）
-- 注：主路径走 Redis SADD 原子去重，MySQL 仅做审计持久化
-- =====================================================
...
- **MongoDB 评论集群（32分片 × 1主2从）**：存储评论主集合，按 object_id 哈希分片（业界最佳实践：文档模型天然适配评论树形结构+富文本扩展）；
...
L3 MongoDB（最终持久化，毫秒到秒级）：
   └── 作为评论数据源和对账基准，极少量请求穿透到此
```
...
```
问题：评论列表有 100 万条，OFFSET 1000000 性能极差（传统 DB 全表扫描）

OFFSET 分页的问题：
db.comment.find({ object_id: ? }).sort({ create_time: -1 }).skip(100000).limit(20)
-- 即便 MongoDB，skip 大偏移量仍需遍历跳过前 100000 条，时间复杂度 O(offset)，深分页慢到几秒

游标分页（本系统方案）：
db.comment.find({ object_id: ?, create_time: { $lt: cursor } }).sort({ create_time: -1 }).limit(20)
-- 利用 (object_id, create_time) 复合索引，直接定位到游标位置，O(log N)，恒定性能
...
### 10.2 MongoDB 评论集群水平扩展

```
当前：按 object_id 哈希分片（同一内容的所有评论在同一 chunk）
问题：热点内容（如顶流明星动态）评论集中在少数 chunk，形成热点分片

优化方案（MongoDB 原生能力，无需中间件）：

① 自动均衡（日常）：
   - MongoDB balancer 自动检测 chunk 大小不均衡
   - 超过 chunkSize（默认 128MB）自动拆分，跨分片迁移
   - 业务零感知，无需人工干预

② 热点内容独立集合隔离（极端场景）：
   - 热点内容（评论数 > 100万）自动迁移到独立集合 comment_hot
   - 独立集合部署在专属分片组（物理隔离，不影响普通内容）
   - 路由层从 ETCD 读取热点列表，命中 → 路由到热点集合

③ 在线加分片（容量扩展）：
   - sh.addShard("shard-new:27017")，balancer 自动迁移 chunk
   - 扩容全程业务正常读写，无需双写/停服
   - 对比 MySQL 分库扩容：MySQL 256库→512库需双写+路由切换（1周窗口期）
     MongoDB 加分片后 balancer 自动完成，无人工介入

④ 预分裂（大事件预案）：
   - 大型事件前提前对热点 object_id 执行 sh.splitAt() 预分裂 chunk
   - 确保热点内容的 chunk 分散在多个分片上
```

### 10.3 冷热评论分层存储

```
热数据（0~30天） : MongoDB + Redis（在线实时查询，快速响应，P99 < 50ms）
温数据（30天~1年）: MongoDB 低优先级副本集（读写分离，按需加载到 Redis，P99 < 200ms）
...
触发方式：MongoDB TTL 索引自动过期 + 定时任务每日凌晨，将 create_time < 30天前的评论导出归档
效果：MongoDB 主集群只保留 30天热数据，单分片数据量可控
...
│  ├── 全量评论读写服务                 │  │  ├── 全量评论读写服务                │
│  ├── MongoDB 16分片（按 object_id 归属）│  │  ├── MongoDB 16分片（互为异地副本）   │
...
数据同步：
  - MongoDB 跨地域副本集（Zone-aware Sharding）或 Change Stream 双向同步，冲突策略：以写主分片为准
...
【冷数据归档（每日凌晨）】
  □ 将 30天前评论导出到归档库
  □ 校验归档数据完整性（条数对比）
  □ 清理 MongoDB 主集群已归档数据（TTL 自动过期或手动删除）
  □ 更新路由规则（30天前查询走归档库）
...
### Q1：评论系统采用两级评论树（根评论 + 二级回复），查询某条根评论下的所有回复需要按照 root_id 查询，但评论集合是按 object_id 分片的，同一根评论的回复可能散落在不同分片上，如何解决？

**参考答案：**

**核心：分片键的选择决定了哪类查询是快路径，哪类是跨分片查询。本系统的选择是"读评论列表"是主路径，按 object_id 哈希分片最优；回复查询通过同分片设计自然解决。**

**问题分析：**
- 按 `object_id` 分片：同一内容的所有评论（根评论+回复）在同一 chunk → 根评论列表查询快，但单热点内容分片压力大
- 按 `comment_id` 分片：均匀分散，但根评论的回复散落各分片 → 回复查询需要跨分片

**本系统选择 `object_id` 哈希分片，并解决热点问题：**

① **回复查询（同分片内）**：
```javascript
// 根评论和其回复在同一分片（object_id 相同），不跨分片
db.comment.find({ root_id: comment_id }).sort({ create_time: 1 }).limit(20)
// root_id 有索引，同分片内查询，O(log N)，不跨分片（按 object_id 哈希分片）
```

② **热点内容分片隔离**：
- 问题：热点内容的所有评论在同一 chunk，形成热点分片
- 解决：热点内容（评论数 > 100万）动态迁移到独立热点集合（详见可扩展性章节 §10.2）
- 路由层按 ETCD 热点列表决定路由到普通集合还是热点集合
...
**① 数据同步架构：**
```
MongoDB 评论集合（写入） → MongoDB Change Stream（监听 oplog）→ MQ → ES 同步消费服务 → Elasticsearch 索引

延迟：< 5s（Change Stream → ES 同步延迟）
