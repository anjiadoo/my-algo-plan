...
| 章节 | 核心内容 |
|------|---------|
| **需求与边界** | 功能边界（发布/查看/回复/点赞/删除/审核）、二级评论树结构、明确禁行 |
| **容量评估** | DAU 6亿闭环推导、500万 QPS 读/50万 QPS 写分层计算、存储/带宽/Redis/DB/MQ 完整推导 |
| **库表设计** | 按"实体+事件+读模型"三类建模（实体 Comment/Like、事件 CommentPublished/Deleted/Liked/AuditFinished、读模型 CommentCountView/CommentListView/AuditRecord/Notification）、5张核心表完整 SQL（评论主表/回复表/计数表/点赞表/审核表） |
| **整体架构图** | Mermaid flowchart + 四层文字描述（接入/应用/中间件/存储），核心设计原则三条 |
| **核心流程** | 发布评论（审核前置+事务补偿）、读取评论列表（游标分页+多级缓存）、点赞（Redis 原子计数）、热点内容评论爆炸处理 |
| **缓存架构** | 三级缓存（go-cache/Redis/DB）、评论列表 ZSet 设计、热点评论独立集群、缓存穿透/击穿/雪崩全覆盖 |
| **MQ 设计** | 5个 Topic 分区数完整推导、审核异步化、通知解耦、计数最终一致性 |
| **核心关注点** | 热点内容评论爆炸（分片+本地聚合）、防刷/防重（三层幂等）、计数强一致（Redis+DB双写）、游标分页 vs OFFSET |
| **容错性设计** | 分层限流/熔断阈值/三级降级/ETCD 动态开关/五类故障兜底矩阵 |
| **可扩展性** | 评论树水平拆分、冷热评论分层存储、异地多活流量染色 |
| **高可用运维** | 评论核心指标监控（发布成功率/读 P99/计数误差）、三级告警、热点预案 |
| **面试高频10题** | Q1~Q10 均为评论场景定制（评论树展开/游标分页/计数一致性/热点爆炸/审核时序/点赞精度/删除可见性/AT通知/跨服务聚合/冷热归档） |
