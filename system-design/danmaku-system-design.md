...
| 章节 | 核心内容 |
|------|---------|
| **需求与边界** | 点播弹幕 + 直播弹幕双场景、发送/拉取/漫游/审核/互动、明确禁行（禁止长连接回源 DB、禁止同步落盘阻塞推送、禁止全量广播、禁止无限回溯） |
| **容量评估** | DAU 4亿闭环推导、点播拉弹幕 800万 QPS / 直播推送 3亿条/秒扇出、带宽 / 存储 / Redis / MQ 完整闭环 |
| **库表设计** | 6张核心表（弹幕主表按 video_id + 时间桶分片、直播弹幕冷热表、审核表、用户关系禁言表、计数表、屏蔽词表），SQL 带分片规则与索引设计 |
| **整体架构图** | Mermaid flowchart + 四层文字描述（接入 / 网关 WebSocket / 应用 / 存储），推拉混合（点播拉 + 直播推）核心设计原则 |
| **核心流程** | 点播发弹幕（审核前置+按视频分桶落 Redis ZSet+异步落 DB）、点播拉弹幕（按播放时间窗口拉取+客户端渲染调度）、直播长连接发弹幕（网关路由房间+分片合并+抽样广播）、直播推送风暴抑制（合并/抽样/降级四层） |
| **缓存架构** | 四级缓存（端本地缓存 → CDN 静态弹幕包 → 房间本地内存 → Redis ZSet），点播冷热分层、直播"最近30秒"滑动窗口、热点视频弹幕包 CDN 预热 |
| **MQ 设计** | 5个 Topic 分区数推导（发送异步落库/审核/计数/房间扇出/死信），事务消息保证"推送成功 → 落盘成功"最终一致，消费者幂等 + 房间粘滞路由 |
| **核心关注点** | 热点房间百万在线（房间分片+Gateway 粘滞+抽样降采样）、弹幕洪水抑制（令牌桶+合并窗口+智能抽样）、防刷防垃圾（短时长/重复/同人/风控AI四层）、幂等方案（弹幕ID雪花+Redis NX）、时序对齐（点播相对时间+直播绝对 ts） |
| **容错性设计** | 分层限流（接入 / 房间 / 用户三维）、熔断阈值（下游审核熔断丢入低优先级队列）、三级降级（关互动/关花式弹幕/关推送只保点播）、ETCD 动态开关、五类故障兜底矩阵 |
| **可扩展性** | Gateway 无状态+房间一致性哈希迁移、Redis 按房间 slot 在线扩容、弹幕主表时间分区滚动归档（热 MySQL → 冷 TiKV/HBase → OSS）、MQ 分区弹性扩容 |
| **高可用运维** | 5大类监控（推送延迟 P99 / 弹幕掉帧率 / Gateway 连接数 / 审核延迟 / 房间热点分布）、三级告警、开播前预热 checklist、大型赛事护航规范 |
| **面试高频10题** | Q1~Q10 均为弹幕场景定制（百万房间扇出/点播按时间拉取/重复弹幕去重/Gateway 断连续连/审核异步可见性/长连接鉴权/冷热归档/弹幕时序一致/推送抽样策略/房间扩缩容数据迁移） |
...
### Q1：你说单房间 500 万在线通过 Gateway 分副本广播，副本之间用 Redis PubSub 同步。那 Redis PubSub 单频道吞吐上限是多少？如果达到上限怎么办？你如何验证这个容量够用？
...
### Q4：直播场景 Gateway 长连接掉线重连（比如 4G 切 WiFi），客户端需要拉回断线期间的弹幕。怎么设计这个续传机制？如何防止重复渲染和漏弹幕？
...
### Q6：Gateway 单机 5 万长连接，但每个连接持有一条 TCP，goroutine（Go）或 thread（其他语言），内存怎么算？你如何把内存优化到能支持单机 5 万连接？
...
### Q10：大型赛事前预期单房间瞬时涌入 500 万人，你的 Gateway 副本从日常 20 个扩到 100 个，这个过程怎么做？扩容中断会造成什么问题？
