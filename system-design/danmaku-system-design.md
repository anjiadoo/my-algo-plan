...
```go
// 业界典型实现：per-connection 令牌桶（进程内存）
type ConnRateLimiter struct {
    // 令牌桶（字节/快手方案，更平滑）
    bucket     *rate.Limiter  // Go 标准库 x/time/rate
    // rate.NewLimiter(rate.Every(2*time.Second), 1)  // 2秒1条，桶容量1
}

// Gateway 收到消息时直接判断（零网络开销）
func (g *Gateway) onClientMsg(conn *Conn, msg *DanmuMsg) {
    if !conn.limiter.Allow() {
        // 直接丢弃，不透传到下游发送服务
        conn.WriteErr(ErrRateLimited)
        return
    }
    // 通过本地频控，转发到弹幕发送服务
    g.sendService.Forward(msg)
}
```

**为什么要做本地频控（而不是全依赖全局频控）：**

| 维度 | 本地频控 | 全局频控（Redis） |
|------|---------|----------------|
| 延迟 | < 0.01ms（内存操作） | 1~3ms（网络 RTT） |
| 适用场景 | **第一道削峰**，拦截恶意刷屏用户 | 精确的全局用户级限制 |
| 容量 | 无上限（每连接一个计数器） | 受 Redis QPS 限制 |
| 核心作用 | 保护下游，500万连接若都打到 Redis 就 DDoS 自己了 | 跨节点一致的速率控制 |

**业界实践（B站/字节/快手）：**
- **B站 goim**：每个连接一个 `lastMsgTime` 字段，距上次 < 2s 直接丢弃
- **字节直播**：用 Go `x/time/rate` 标准库令牌桶，支持"突发容忍"（允许连续发 2 条但之后冷却 4s）
- **快手**：滑动窗口计数器，10s 窗口最多 5 条
...
```lua
-- 业界最常用方案：Redis Lua 滑动窗口（原子操作，避免竞态）
-- Key: rate:user:{uid}
-- 实现：ZSET 滑动窗口 or 简单 INCR+EXPIRE

-- 方案一：简单固定窗口（B站早期）
-- INCR rate:user:{uid} → if > 5 then reject
-- EXPIRE rate:user:{uid} 10

-- 方案二：滑动窗口（字节/腾讯方案，更精确）
local key = KEYS[1]           -- rate:user:{uid}
local window = tonumber(ARGV[1])  -- 窗口大小 10000ms
local limit = tonumber(ARGV[2])   -- 限制次数 5
local now = tonumber(ARGV[3])     -- 当前时间戳ms
...
**另一种业界常用方案——Redis Cell（令牌桶模块）：**
```
# Redis 4.0+ 模块，单命令实现令牌桶
CL.THROTTLE rate:user:{uid} 5 5 10
# 含义：桶容量5，每10秒补充5个令牌
# 返回：[是否允许, 桶容量, 剩余令牌, 若拒绝需等待秒数, 桶满所需秒数]
```
...
客户端发弹幕
    ...
    ▼
┌─────────────────────────────────────────┐
│  Gateway 本地频控（令牌桶，进程内存）      │ ← 第一道：拦截 95% 恶意流量
│  单连接 1条/2s，超则直接丢弃              │    不消耗 Redis QPS
└─────────────────────┬───────────────────┘
                      │ 通过
                      ▼
┌─────────────────────────────────────────┐
│  弹幕发送服务 - Redis 全局频控            │ ← 第二道：跨节点精确控制
│  滑动窗口 10s/5条（普通）或 10s/10条(VIP)  │    解决"用户切连接绕过本地频控"
└─────────────────────┬───────────────────┘
                      │ 通过
                      ▼
┌─────────────────────────────────────────┐
│  房间级频控（可选第三道）                   │ ← 保护房间下游：单房间 > 10000/s
│  Redis INCR room:rate:{room_id}          │    触发房间级降级
└─────────────────────────────────────────┘
```

**为什么全局频控必须存在：**
- 用户可能连接到不同 Gateway 节点（重连、负载均衡切换）
- 本地频控只管"单连接"，如果用户开多个连接或频繁重连，本地计数器清零
- 全局频控用 `user_id` 作 Key，无论连到哪里都统一计数
...
**这个理解是错误的。** 客户端**只连接到 1 个 Gateway 节点**，不是 100 个。

```
                    ┌─ Gateway 0 ─── 5万用户（各自最多收 200条/s）
                    ├─ Gateway 1 ─── 5万用户（各自最多收 200条/s）
500万用户 ───────────├─ Gateway 2 ─── 5万用户
   │                ├─ ...
   │                └─ Gateway 99 ── 5万用户
   │
   └─→ 每个用户只连到其中 1 个 Gateway！
        不是连到 100 个！
```
...
大房间发送 20万条/s
        ...
        ▼
┌────────────────────────────────────────────┐
│  RocketMQ 广播 Topic（256 Partition）       │
│  每条消息只被 1 个 Job 消费（集群消费模式）     │
└────────────────────┬───────────────────────┘
                     │
                     ▼
┌────────────────────────────────────────────┐
│  32 个 Job 实例（每实例消费 8 Partition）     │
│  每个 Job 查路由表，将消息 gRPC 推送给         │
│  所有持有该房间连接的 Gateway 节点             │
└────────────────────┬───────────────────────┘
                     │ gRPC 推送（每个 Gateway 收到全量或预抽样后的消息）
                     ▼
┌────────────────────────────────────────────┐
│  Gateway 节点 X（该房间在本节点有5万连接）      │
│                                            │
│  收到 20万条/s 全量房间消息                   │
│       │                                    │
│       ▼                                    │
...
**关键认知：抽样发生在"最后一公里"（Gateway→单连接），不是在"汇聚层"。** 文档中"5s滑动窗口，超1000条/s 按权重抽样保留30%"的正确含义是：当一个 Gateway 节点收到的某房间消息超过 1000条/s 时，启动抽样逻辑，最终每个连接收到的量控制在 200条/s。
...
层级 1：发送侧限流
───────────────────────────────────
• 全站 200万条/s 发送，是所有房间合计
• 单房间 20万条/s（顶级赛事高潮期）
• 本质：源头不可控（用户行为），必须在推送侧削减

层级 2：Job 路由层（可选削减点，业界部分公司在此抽样）
───────────────────────────────────
• 字节方案：Job 层先做"房间级预抽样"
  - 如果 room 消息率 > 5000条/s，Job 在转发前就抽样到 2000条/s
  - 所有 Gateway 收到的已经是抽样后的结果
  - 优点：减少 Job→Gateway 的 gRPC 带宽
  - 缺点：所有客户端看到完全相同的抽样结果（不够个性化）

层级 3：Gateway 节点级抽样（本文档中的方案）
───────────────────────────────────
• 每个 Gateway 收到全量（或 Job 预抽样后的量）
• Gateway 内部维护每房间的滑动窗口计数器
• 超阈值时启动智能抽样：
  - 优先级队列：礼物/VIP/高互动 > 普通弹幕
  - 内容去重/合并：相同内容聚合为 "x 1.2万"
  - 最终保留 ≤ 200条/s 推给每个连接
...
| 公司 | 推送侧核心抽样点 | 策略 |
|------|----------------|------|
| **B站** | Comet（Gateway）层 | 每房间维护环形 buffer（1000条），超满时按优先级淘汰旧消息；推给连接时按每秒上限裁剪 |
| **字节直播** | Job 层 + Gateway 层**两级抽样** | Job 层做"房间全局抽样"（降到 2000条/s），Gateway 层再做"连接级个性化裁剪"（VIP 看得多，普通用户看得少） |
| **快手** | Gateway 层 | 单房间消息进入"优先级堆"，每 50ms 从堆顶取 10 条推送，低优先级自动过期丢弃 |
| **腾讯/虎牙** | 边缘节点（类似 CDN 转发层） | 在 Gateway 前加一层"消息边缘节点"，按地域/运营商分组，每组节点做独立抽样后推给下游 Gateway |
...
│                                              │
│  房间消息率 > 5000条/s 时触发：                  │
│  • 优先级 Top-K 保留（礼物/VIP 100% 保留）       │
│  • 普通弹幕按权重抽样，总量控制在 2000条/s        │
│  • 重复内容合并（"GGGG" x 5000 → 1条聚合消息）   │
│                                              │
│  抽样后 2000条/s 推送给 100 个 Gateway           │
│  （而非 20万条/s，gRPC 带宽降 100 倍！）          │
└──────────────────────┬───────────────────────┘
                       │ 2000条/s（已大幅削减）
                       ▼
┌──────────────────────────────────────────────┐
│  Gateway 节点 - 第二级抽样（连接级个性化）        │
│                                              │
│  收到 2000条/s，按用户属性个性化裁剪：            │
│  • VIP用户：保留 200条/s（密度高）               │
│  • 普通用户：保留 100条/s（密度中）              │
│  • 新用户/低活跃：保留 50条/s（密度低）           │
│                                              │
│  结果：每个连接 50~200条/s，用户屏幕可阅读        │
└──────────────────────────────────────────────┘
```

**两级抽样的优势：**
1. **Job 层预抽样**大幅降低内部链路带宽：20万→2000条/s，gRPC 带宽降 100 倍
2. **Gateway 层个性化裁剪**保证不同用户体验差异化：VIP 看到更多内容
3. **客户端本身还有最终兜底**（丢弃超出渲染能力的部分）

**带宽闭环验证：**

```
如果不做 Job 层预抽样（本文档当前方案）：
• Job→Gateway 带宽：20万条/s × 100 Gateway × 250B = 5 GB/s/Job（需万兆网卡）
• Gateway→客户端带宽：500万 × 200条/s × 250B = 250 GB/s ≈ 2 Tbps

如果采用两级抽样（字节方案优化）：
• Job→Gateway 带宽：2000条/s × 100 Gateway × 250B = 50 MB/s（很轻）
• Gateway→客户端带宽：还是 500万 × 200条/s × 250B = 250 GB/s（瓶颈在最后一公里）

结论：出口带宽是不可避免的（500万人每人收200条），
     但内部链路带宽可以通过 Job 层预抽样大幅降低。
```
...
| 问题 | 核心答案 |
|------|---------|
| 本地频控 | **进程内存令牌桶**，per-connection，<0.01ms，第一道削峰拦截 95% 恶意流量，保护下游 Redis |
| 全局频控 | **Redis Lua 滑动窗口**（或 Redis Cell 模块），per-user_id 全局一致，解决"切连接/多连接绕过本地频控"问题 |
| 节点级抽样"汇聚到客户端很多" | **误解**——客户端只连 1 个 Gateway，不是 100 个；业界最佳实践是 **Job 层全局预抽样（20万→2000条/s）+ Gateway 层连接级个性化裁剪（→200条/s）+ 客户端硬上限兜底** |
...
```
                                    ┌──────────────┐
                                    │ 实时推送服务  │ ← 需要抽样（热门房间1/50）
                                    ├──────────────┤
            ┌─────────┐            │ 弹幕存储/回放 │ ← 需要全量（否则录播弹幕稀疏）
用户发送 →  │   MQ    │ ──fan out→ ├──────────────┤
            └─────────┘            │ 内容审核服务  │ ← 需要全量（漏审=监管风险）
                                    ├──────────────┤
                                    │ 数据分析/BI  │ ← 需要全量（统计热词/情绪）
                                    ├──────────────┤
                                    │ AI训练数据   │ ← 需要全量
                                    └──────────────┘
```
...
```
监管要求：所有用户发送的弹幕必须100%过审、100%存储可追溯
...
这是**不可商量的硬约束**，一票否决了 MQ 前抽样的方案。

**2. 回放/录播需要全量弹幕**

用户看直播回放时，期望看到完整的弹幕体验。如果入 MQ 前就丢了，回放弹幕密度会不可接受地低：

```
直播时：每秒5000条弹幕 → 抽样1/50 → 推送100条/秒（观众体验OK）
回放时：只存了100条/秒 → 弹幕极其稀疏（用户体验差）

正确做法：全量存储，回放时重新按密度策略加载
```

**3. MQ 不是瓶颈，推送才是——ROI不划算**

```
系统瓶颈分析：

Kafka 吞吐：单集群轻松 百万条/秒，几GB/秒
   → 弹幕消息体很小（~200Byte），百万条≈200MB/s，对Kafka不是压力

真正的瓶颈：推送给500万在线连接
   → 1条弹幕 × 500万连接 = 500万次网络IO
   → 这才是系统扛不住的地方

MQ带宽成本 vs 推送带宽成本：
├── MQ: 百万条/秒 × 200B = 200MB/s（内网带宽，几乎免费）
└── 推送: 100条/秒 × 500万连接 × 200B = 100GB/s（这才是大头）

在MQ前抽样：省了 200MB/s（九牛一毛）
...
| 机制 | 目的 | 时机 | 丢弃对象 |
|------|------|------|---------|
| **频控(Rate Limit)** | 拦截恶意/刷屏用户 | MQ之前（Gateway） | 超频的那些消息 |
| **抽样(Sampling)** | 控制下发密度 | MQ之后（Push服务） | 正常消息的一部分 |

```
频控（MQ前）：用户2秒内发了10条 → 只放行1条，其余9条是"非法"的
抽样（MQ后）：1000个正常用户各发1条 → 只推送其中20条给观众

频控丢弃的是"不该存在的消息"——丢了不心疼，也不需要存储/审核
...
```
MQ前做频控 ✓（拦截非法流量，保护系统）
MQ前做抽样 ✗（会导致审核漏检、回放缺失、分析失真）

正确的抽样位置：MQ消费端（Push服务），只影响"实时推送"这一条链路
其他链路（存储、审核、分析）消费全量数据，互不干扰
```

这就是**职责分离**的体现——MQ 作为"数据总线"保证完整性，各消费者按自己的需要独立决定取舍策略。
