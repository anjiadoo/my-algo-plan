...
| 章节 | 核心内容 |
|------|---------|
| **一、需求与边界** | 点播弹幕 + 直播弹幕双场景、发送/拉取/漫游/审核/互动、明确禁行（禁止长连接回源 DB、禁止同步落盘阻塞推送、禁止全量广播、禁止无限回溯） |
| **二、容量评估** | DAU 4亿闭环推导、点播拉弹幕 800万 QPS / 直播推送 3亿条/秒扇出、带宽 / 存储 / Redis / MQ 完整闭环 |
| **三、库表设计** | 6张核心表（弹幕主表按 video_id + 时间桶分片、直播弹幕冷热表、审核表、用户关系禁言表、计数表、屏蔽词表），SQL 带分片规则与索引设计 |
| **四、整体架构图** | Mermaid flowchart + 四层文字描述（接入 / 网关 WebSocket / 应用 / 存储），推拉混合（点播拉 + 直播推）核心设计原则 |
| **五、核心流程** | 点播发弹幕（审核前置+按视频分桶落 Redis ZSet+异步落 DB）、点播拉弹幕（按播放时间窗口拉取+客户端渲染调度）、直播长连接发弹幕（网关路由房间+分片合并+抽样广播）、直播推送风暴抑制（合并/抽样/降级四层） |
| **六、缓存架构** | 四级缓存（端本地缓存 → CDN 静态弹幕包 → 房间本地内存 → Redis ZSet），点播冷热分层、直播"最近30秒"滑动窗口、热点视频弹幕包 CDN 预热 |
| **七、MQ 设计** | 5个 Topic 分区数推导（发送异步落库/审核/计数/房间扇出/死信），事务消息保证"推送成功 → 落盘成功"最终一致，消费者幂等 + 房间粘滞路由 |
| **八、核心关注点** | 热点房间百万在线（房间分片+Gateway 粘滞+抽样降采样）、弹幕洪水抑制（令牌桶+合并窗口+智能抽样）、防刷防垃圾（短时长/重复/同人/风控AI四层）、幂等方案（弹幕ID雪花+Redis NX）、时序对齐（点播相对时间+直播绝对 ts） |
| **九、容错性设计** | 分层限流（接入 / 房间 / 用户三维）、熔断阈值（下游审核熔断丢入低优先级队列）、三级降级（关互动/关花式弹幕/关推送只保点播）、ETCD 动态开关、五类故障兜底矩阵 |
| **十、可扩展性** | Gateway 无状态+房间一致性哈希迁移、Redis 按房间 slot 在线扩容、弹幕主表时间分区滚动归档（热 MySQL → 冷 TiKV/HBase → OSS）、MQ 分区弹性扩容 |
| **十一、高可用运维** | 5大类监控（推送延迟 P99 / 弹幕掉帧率 / Gateway 连接数 / 审核延迟 / 房间热点分布）、三级告警、开播前预热 checklist、大型赛事护航规范 |
| **十二、面试高频10题** | Q1~Q10 均为弹幕场景定制（百万房间扇出/点播按时间拉取/重复弹幕去重/Gateway 断连续连/审核异步可见性/长连接鉴权/冷热归档/弹幕时序一致/推送抽样策略/房间扩缩容数据迁移） |
...
| 3 | **直播房间分片** | 单房间 Gateway 副本 = ⌈在线数/5万⌉，副本间 Pub/Sub | 单 Go Gateway 进程稳定承载5万长连接（epoll + goroutine 池），副本内直接内存广播零网络开销 |
...
| 8 | **连接鉴权** | JWT + 房间 Ticket（短期有效），Gateway 握手校验一次后进入长连 | 百万连接握手若每次查 Redis/DB 必挂；Ticket 由业务网关签发（1分钟有效），Gateway 仅本地 RSA 公钥校验 |
...
- **禁止在 Gateway 进程内落盘**：Gateway 只做 I/O 复用，业务处理交下游
...
> 说明：**"入口 3 亿"是 Gateway 内存广播的逻辑扇出量**（决定 Gateway CPU/内存规模）；**"出口 1 亿"是经抽样/合并后真正下发到客户端的量**（决定出口带宽）。两个数字在不同层级使用，不能混用。
...
- 在线用户房间映射（Gateway Session Registry）：500万峰值连接 × 200B = **1 GB**
- 违规墓碑 Set + 屏蔽词缓存：**10 GB**
- 合计热点数据 ≈ **730 GB**，叠加主从复制 + 碎片 + 扩容余量 ×4 ≈ **3 TB** ✓

**MQ 容量：**
- 峰值消息写入：直播发送 200万 + 点播发送 50万 + 扇出任务 5万（每房间一个扇出任务，Gateway 内部扩散）+ 审核 250万 + 计数 250万 ≈ **750万条/s**
...
**机型标准：** 8核16G Go 服务；Gateway 为 16核32G（长连接内存+goroutine 开销）
...
| 直播 Gateway | 5万长连接/节点 | 500万峰值在线 | **150 台** + 50 台热备 | epoll + goroutine 池，每连接 32KB 内存预算 |
| 扇出 Worker | 2万条/s | 3亿条/秒 | **—** | 由 Gateway 副本内存广播完成，不需独立 Worker |
...
| **DanmuTakedown** 弹幕下架事件 | 违规弹幕被下架 | 弹幕ID、目标ID（视频/房间）、下架原因 | 审核判定违规或用户举报被确认 | ① Redis 墓碑 Set 写入 ② **直播场景 PubSub 广播删除指令**（Gateway 实时隐藏）③ 点播下次拉取客户端差集过滤 |
| **UserMuted** 用户禁言事件 | 主播/房管禁言某用户 | 房间ID、被禁言用户ID、操作人ID、到期时间戳、禁言原因 | 禁言操作成功 | ① 写入 `user_mute` ② Redis 禁言缓存 ③ Gateway 踢出对应用户的连接 |
...
2. **Gateway 层无业务逻辑**：仅做协议解码、鉴权、路由、房间内广播，内存占用线性可预期
...
```
1. 客户端 → API 网关 → 弹幕发送服务(HTTPS)
2. 发送服务:
   2.1 鉴权(JWT)、用户黑名单校验
   2.2 Redis 频控 Lua:
       local k = "rate:uid:"..uid
       local n = redis.call("INCR", k)
       if n == 1 then redis.call("EXPIRE", k, 5) end
       return n <= 1
   2.3 同步关键词审核(AC自动机,本地内存,<2ms)
       - 命中高危词(政治/黄暴)→ 直接拒绝,返回模糊提示
       - 命中中危词(广告/违禁)→ 标记 audit_level=0,先入库后审
   2.4 生成 danmu_id(雪花算法),组装弹幕对象
   2.5 Redis ZSET 写入:
       ZADD danmu:vod:{video_id}:{bucket} offset_ms danmu_id
       HSET danmu:content:{video_id} {danmu_id} <content_packed>
       该 bucket_id = offset_ms / 30000
   2.6 发送 RocketMQ: topic_danmu_persist (落盘) + topic_danmu_audit (AI审核)
       使用 RocketMQ 事务消息: 本地写 Redis 成功后半消息提交
   2.7 返回客户端成功(端到端 < 100ms)
3. 客户端乐观渲染: 立即在本地时间轴叠加该弹幕,2s 未收到 ACK 回滚展示
```
...
```
客户端每 30s 请求一次,参数: video_id, start_offset_ms, end_offset_ms

1. 计算需要的 bucket 列表: [start/30000, end/30000]
2. 先查 CDN:
   GET /danmu/v1/{video_id}/{bucket_id}.pb (Protobuf打包)
   - CDN 命中(95%) 直接返回,边缘节点 5ms
   - 未命中回源到拉取服务
3. 拉取服务回源:
   3.1 Redis ZRANGEBYSCORE danmu:vod:{video_id}:{bucket} start end
       ZREVRANGE 按时间取 TopN(默认单桶 500 条上限)
   3.2 HMGET 批量取内容
   3.3 Redis SMEMBERS danmu:tombstone:{video_id} 做本地差集
   3.4 冷数据(不在 Redis 中)→ MySQL 按 idx_video_bucket 查询
       若 video 在归档区 → 查 TiKV
   3.5 Protobuf 打包响应,写 CDN 边缘缓存(TTL 5min)
4. 客户端按 offset_ms 时间线组织队列,渲染调度器按帧插入画面
```
...
```
长连接已建立的前提: 客户端进房 → 业务网关签发 Ticket → 建 WS 连 Gateway → Gateway 将该连接注册到 roomRegistry[room_id]

1. 客户端 WS 发送 msg{type=danmu, content=...}
2. Gateway:
   2.1 本地频控(令牌桶,单连接 1条/2s)
   2.2 若超过本地阈值 → 直接丢弃,不打下游(第一道削峰)
   2.3 Gateway 将消息 HTTP/gRPC 转发给弹幕发送服务
      (采用 unix socket 或本机 sidecar,减少网络开销)
3. 发送服务:
   3.1 关键词审核(同步)
   3.2 Redis 频控
   3.3 Redis ZADD danmu:live:{room_id} server_ts danmu_id,ZREMRANGEBYSCORE 清理 > 30min 数据
   3.4 发布房间消息:
       PUBLISH room_channel:{room_id} <danmu_payload>
   3.5 MQ 异步: topic_danmu_live_persist, topic_danmu_audit
4. Gateway 接收 PubSub:
   4.1 每个 Gateway 节点只订阅"本节点有连接的房间频道"
       (Gateway 启动时注册, SUBSCRIBE room_channel:{room_id})
   4.2 节点内遍历 roomRegistry[room_id] 的连接, 内存直接 write 消息
   4.3 在节点级再做一次"合并与抽样":
       - 5s 滑动窗口,若 > 1000条/s: 按用户权重抽样保留 30%
       - 重复内容合并(相同 content 10 条以上 → "x 1.2万")
       - 优先级队列: 礼物/上舰/VIP > 普通 > 抽样内容
5. 客户端接收: 按 server_ts 排队,渲染器每帧推入(最多 300条/s 防卡)
```

**房间副本机制：**
- 单房间单 Gateway 节点最多 5 万连接，超过自动触发"副本扩展"
- 用一致性哈希 `hash(room_id + replica_idx) → Gateway 节点`，`replica_idx` 随上线人数增长
...
3. 若判定违规:
   ...
   3.3 直播场景额外: PUBLISH room_channel:{room_id} {type:"delete", id:danmu_id}
       Gateway 收到后向房间内连接广播删除指令, 客户端立即隐藏
...
| L3 | Gateway 进程内存 | 直播房间最近 1000 条 | 房间生命周期 | 用户断连重连秒回弹幕 |
...
- 断连重连：客户端带上 `last_server_ts`，Gateway 从 Redis ZSet 回放增量
- 删除：PubSub 广播删除事件，Gateway 级别广播到连接，实时隐藏
...
- Gateway 层的房间本地缓存可在 Redis 全挂时独立工作 5 分钟
...
2. **Gateway 副本**：按在线数自动扩副本，`replica = ⌈online/50000⌉`
3. **节点内合并**：同一 Gateway 节点内 5s 窗口内重复内容合并
4. **智能抽样**：大于阈值时按"礼物/VIP/高权重用户"保留，普通弹幕降采样到 30%

**计算示例：**
```
头部房间 500 万在线, 峰值发送 20万条/s
→ Gateway 副本 100 个, 每副本 5万连接
...
- **直播**：弹幕按 `server_ts` 排序
  - NTP 矫正：Gateway 握手时下发服务器时间，客户端本地偏移
...
| 故障 | 兜底方案 |
|-----|---------|
| Redis 全挂 | 点播切 DB 直读（仅允许前 5 分钟冷启动）；直播仅靠 Gateway 内存广播（丢失历史回放） |
| MQ 挂 | 发送服务写本地磁盘 WAL，恢复后补投 |
| Gateway 集群故障 | 直播链路降级为 HTTP 轮询（3s 一次） |
...
- **多机房**：3 IDC 同城三活 + 异地灾备
- **直播链路**：Gateway 跨机房，DNS 智能解析就近接入；Redis PubSub 跨机房单向复制，故障切换丢失 < 3s 消息可接受
...
| 类 | 指标 | 告警阈值 |
|----|------|---------|
| 流量 | 发送 QPS、拉取 QPS、Gateway 连接数 | 超 80% 容量 P1 |
| 性能 | 推送端到端延迟 P99、拉取 P99、审核延迟 | 推送 > 1.5s P0 |
| 客户端 | 弹幕渲染掉帧率、发送成功率 | 掉帧 > 20% P1 |
| 一致性 | 墓碑 Set 扩散延迟、落盘延迟 | 落盘堆积 > 5min P1 |
| 资源 | Redis CPU/QPS、Gateway goroutine、MQ 堆积 | CPU > 80% P1 |
| 业务 | 违规识别率、机器人拦截率 | 违规下架 < 99% P2 |

### 分级告警

- **P0（5 分钟）**：推送不可用、Gateway 宕机、Redis 主挂、违规弹幕扩散
...
**大型赛事护航（世界杯/春晚）：**
- **T-7 天**：容量评估、扩容 Gateway/Redis/MQ、压测至目标 QPS × 1.5
- **T-1 天**：全链路演练、降级开关检查、巡检预案、值班到位
- **T-0 时**：全员监控大盘、禁止发版变更、实时调整抽样率
- **T+1 时**：复盘、数据归档、回顾改进

**日常规范：**
- Gateway 灰度发布：5% → 20% → 50% → 100%，每阶段观察连接数与 goroutine
- Redis 变更必须走双写过渡
...
### 问题1：你说单房间 500 万在线通过 Gateway 分副本广播，副本之间用 Redis PubSub 同步。那 Redis PubSub 单频道吞吐上限是多少？如果达到上限怎么办？你如何验证这个容量够用？
...
② 验证方法：单房间 20万弹幕/s × 100 副本 Gateway 订阅 = 主节点需要 20万 × 100 = 2000万条/s PubSub 出流量，远超单节点上限。

③ 解决方案（生产实际做法）：
- **频道分片**：`room_channel:{room_id}:{shard}`，shard = replica_idx % 8，发送端对每个 shard 写入，每个 Gateway 副本只订阅自己 shard 的频道；20万/s × 8 shard = 每 shard 2.5万/s 可控
- **节点级本地 FanOut**：Gateway 节点订阅后单线程接收、再交由节点内 goroutine 池分发到连接，避免 Redis 直接对接连接
- **高级方案**：直接用自研 Pulsar-like 发布订阅组件，或 Kafka 多消费者组模型替代 PubSub

④ 容量闭环验证：
```
20万条/s × 每条 250B = 50 MB/s 单房间发送带宽
100 Gateway 副本 × 5万连接 = 500万在线 ✓
8 shard × 2.5万/s × 副本侧 5万连接 × 0.3抽样 × 250B = 每副本 2.8 GB/s 出带宽
```
这个数字和前文容量评估闭合。
:::

### 问题2：点播弹幕按 `video_id + bucket`（每 30s）做 Redis ZSet 分片，但有一个热门番剧的某一集开头 30s 弹幕就有 2000 万条，ZSet 根本存不下。你怎么处理？拉取时怎么保证不影响用户体验？
...
### 问题3：你用 RocketMQ 事务消息异步落盘，如果 Redis 扇出成功、MQ 发送成功，但下游落盘消费者 OOM 连续挂了 10 分钟，恢复后会不会丢弹幕？怎么保证"用户看到的弹幕最终都能落盘"？
...
### 问题4：直播场景 Gateway 长连接掉线重连（比如 4G 切 WiFi），客户端需要拉回断线期间的弹幕。怎么设计这个续传机制？如何防止重复渲染和漏弹幕？
...
- Gateway 校验 Ticket 后，将连接注册到 roomRegistry，并将 `last_server_ts` 记录到连接上下文

② 增量拉取：
- Gateway 接受连接后，如果 `last_server_ts > 0` 且 `now - last_server_ts < 30min`：
...
- 服务端采用"推送确认"：客户端每收 100 条回一个 ACK，Gateway 记录连接级 ACK 位点；若断连发现 ACK 位点 < 推送位点，重发差值

⑤ 边界处理：
- 如果客户端本地时钟漂移，`last_server_ts` 不准？—— Gateway 握手时强制返回 server_time，客户端本地偏移校准
- 如果 Redis ZSet 已经 ZREMRANGE 清理了老数据？—— 返回 `history_lost=true`，客户端只展示最新数据，弹"历史弹幕已失效"提示

⑥ 生产细节：字节直播真实场景下，掉线 30s 内重连占比 > 90%，此机制覆盖率极高。
:::

### 问题5：AI 审核是异步的，用户发的违规弹幕可能已经推送给了几十万用户，这怎么下架？下架能不能做到 1s 内所有客户端看不到？

**参考答案：Redis 墓碑 Set + Gateway 广播删除 + 客户端差集过滤。**
...
- Gateway 广播删除时做批量聚合（50ms 合并窗口），减少对连接的写次数

⑤ 真实数据：字节直播审核 P99 延迟 800ms，客户端隐藏 P99 1.2s，极少数超标的是 AI 模型复杂内容（如隐晦政治内容），会进入人审通道。
:::

### 问题6：Gateway 单机 5 万长连接，但每个连接持有一条 TCP，goroutine（Go）或 thread（其他语言），内存怎么算？你如何把内存优化到能支持单机 5 万连接？
...
### 问题7：点播弹幕按 `video_id % 256` 分表，但热门视频（头部 TOP 100）的弹幕占全站 80%，分完仍然热点集中在这 100 张表上，这个分表方案是不是失败了？怎么办？
...
### 问题8：你客户端要"按视频播放时间渲染弹幕"，但用户网络抖动、CDN 延迟、客户端解码延迟都会造成渲染时机和弹幕时间对不上。怎么保证弹幕和视频画面对齐？
...
### 问题9：弹幕系统需要做"智能抽样"让用户屏幕不卡，但哪些弹幕该保留、哪些该丢？如果总有用户抱怨"我看不到我发的弹幕"怎么办？
...
### 问题10：大型赛事前预期单房间瞬时涌入 500 万人，你的 Gateway 副本从日常 20 个扩到 100 个，这个过程怎么做？扩容中断会造成什么问题？
...
② 一致性哈希：
- Gateway 用一致性哈希环，按 `hash(user_id) % ring_size` 决定用户落到哪个副本
- 扩容时只影响 1/N 的用户（N=新副本数），其他用户不断连
- 用户进房时由业务网关签发 Ticket 并带上目标 Gateway IP，客户端直连
...
⑥ 真实案例：字节春晚直播曾用过 500+ Gateway 副本，扩容时间窗口长达 1 周，压测至目标流量 1.5 倍，确保万无一失。
